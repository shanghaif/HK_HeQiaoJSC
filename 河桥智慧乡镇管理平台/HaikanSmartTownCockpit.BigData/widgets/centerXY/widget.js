/* 2017-11-29 09:19:59 | 修改 LYW */
//模块：
mapv3d.widget.bindClass(mapv3d.widget.BaseWidget.extend({
    options: {
        resources: ['view.css'],
        //弹窗
        view: {
    ...
    activate: function () {
        ...
        this.handler.setInputAction(function (movement) {
            ...
            var cartesian = mapv3d.point.getCurrentMousePosition(that.viewer.scene, movement.position);
            if (cartesian) {
                ...
                var wgsPoint = that.viewer.mapv.point2wgs({ x: jd, y: wd }); //坐标转换为wgs
                jd = wgsPoint.x;
                wd = wgsPoint.y;
    ...
    getMapCenter: function () {
        var point = mapv3d.point.getCenter(this.viewer, true);
        return point;
    },
    centerAt: function (_latlng, iscenter) {
        var mpt = this.viewer.mapv.point2map(_latlng); //坐标转换为map一致的坐标系 
    ...
    updateMarker: function (val, iscenter) {
        ...
        if (iscenter) {
            this.viewer.mapv.centerAt({ x: val.x, y: val.y });
        }
