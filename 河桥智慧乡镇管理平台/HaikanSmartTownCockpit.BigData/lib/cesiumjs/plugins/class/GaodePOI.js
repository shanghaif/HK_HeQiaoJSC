...
class GaodePOI {
    ...
    _formatPOIData(arr) {
        ...
        for (var i = 0; i < arr.length; i++) {
            var item = arr[i];
            var coor = [];
            if (item.location && item.location.length>0) coor = mapv3d.pointconvert.gcj2wgs(item.location.split(",")); //坐标
            arrNew.push({
                id: item.id,
                name: item.name, //名称
                x: coor[0],
    ...
    query(opts) {
        ...
        if (entity) {
            if (entity.rectangle) {
                var coors = mapv3d.draw.attr.rectangle.getCoordinates(entity);
                opts.points = coors;
                that.queryPolygon(opts);
            }
            if (entity.polygon) {
                var coors = mapv3d.draw.attr.polygon.getCoordinates(entity);
                opts.points = coors;
                that.queryPolygon(opts);
            }
            if (entity.ellipse) {
                var coor = mapv3d.draw.attr.circle.getCoordinates(entity)[0];
    ...
    queryCircle(opt) {
        ...
        //坐标构造 location
        var cartg = mapv3d.pointconvert.wgs2gcj([opt.x, opt.y]);
    ...
    queryPolygon(opt) {
        ...
        if (coors.length == 2) { //矩形
            var cartg1 = mapv3d.pointconvert.wgs2gcj(coors[0]);
            var cartg2 = mapv3d.pointconvert.wgs2gcj(coors[1]);
            ...
            for (var i = 0; i < newCoors.length; i++) {
                var cartg = mapv3d.pointconvert.wgs2gcj(newCoors[i]);
