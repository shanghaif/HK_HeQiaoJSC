...
namespace HaikanSmartTownCockpit.Api.Controllers.Api.V1.Emergency.Emergencyresponse
{
    ...
    [ApiController]
    [CustomAuthorize]
    public class XunchadutyController : ControllerBase
    {
        ...
        public XunchadutyController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(XunchaDutyRequestPayload payload)
        {
            using (_dbContext)
            {
                var query = from p in _dbContext.XunchaDuty
                            select new
                            {
                                p.XunchaDutyUuid,
                                p.XunchaInfoRen,
                ...
                ToLog.AddLog("查询", "成功:查询:巡查值班信息一条数据", _dbContext);
                return Ok(response);
            }
        }
        #endregion
        ...
        public IActionResult Create(XunchaDutyViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;

            using (_dbContext)
            {
                //if (_dbContext.XunchaDuty.Count(x => x.MemberName == model.MemberName) > 0)
                //{
                //    response.SetFailed("名称已存在");
                //    return Ok(response);
                //}

                var entity = _mapper.Map<XunchaDutyViewModel, XunchaDuty>(model);
                entity.XunchaDutyUuid = Guid.NewGuid();
                entity.IsDeleted = 0;

                entity.AddTime = DateTime.Now.ToString("yyyy-MM-dd");
                entity.AddPeople = AuthContextService.CurrentUser.DisplayName;

                _dbContext.XunchaDuty.Add(entity);
                int res = _dbContext.SaveChanges();
                if (res > 0)
                {
                    ToLog.AddLog("添加", "成功:添加:巡查值班信息数据", _dbContext);
                }
        ...
        public IActionResult Edit(Guid guid)
        {
            using (_dbContext)
            {
                var entity = _dbContext.XunchaDuty.FirstOrDefault(x => x.XunchaDutyUuid == guid);
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map<XunchaDuty, XunchaDutyViewModel>(entity));
                return Ok(response);
            }
        }
        /// <summary>
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(XunchaDutyViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;

            using (_dbContext)
            {
                var entity = _dbContext.XunchaDuty.FirstOrDefault(x => x.XunchaDutyUuid == model.XunchaDutyUuid);
                if (entity == null)
                {
                    response.SetFailed("不存在");
                    return Ok(response);
                }
                //if (_dbContext.XunchaDuty.Count(x => x.TeamName == model.TeamName && x.XunchaDutyUuid != model.XunchaDutyUuid) > 0)
                //{
                //    response.SetFailed("名称已存在");
                //    return Ok(response);
                //}
                ...
                if (res > 0)
                {
                    ToLog.AddLog("编辑", "成功:编辑:巡查值班信息一条数据", _dbContext);
                }
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE XunchaDuty SET IsDeleted=@IsDel WHERE XunchaDutyUUID IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@IsDel", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:巡查值班信息一条数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult XunchadutyImport(IFormFile excelfile)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                string uploadtitle = "巡查安排信息导入" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                try
                {
                    ...
                    ToLog.AddLog("导入", "成功:导入:巡查值班信息数据", _dbContext);
        ...
        /// <returns></returns>
        [HttpGet]
        public IActionResult XunchadutyExport(string ids)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                ...
                var column2 = new List<string>()
                    {
                        "XunchaInfoRen","XunchaAddress","XunchaTime","FuzerPhone"
                    };
                if (!string.IsNullOrEmpty(ids))
                {
                    ...
                    var query = _dbContext.XunchaDuty.Where(x => x.IsDeleted != 1 && parameters.Contains(x.XunchaDutyUuid.ToString())).Select(x => new
                    {
                    ...
                    string sWebRootFolder = _hostingEnvironment.WebRootPath + "\\UploadFiles\\ImportEmergencyExcel\\";
                    string uploadtitle = "巡查安排信息导出" + DateTime.Now.ToString("yyyyMMddHHmmss");
                    ...
                    ToLog.AddLog("导出", "成功:导出:巡查值班信息数据", _dbContext);
                    return Ok(response);
                }
                else
                {

                    var query = _dbContext.XunchaDuty.Where(x => x.IsDeleted != 1).Select(x => new
                    {
                        XunchaInfoRen = x.XunchaInfoRen,
                        XunchaAddress = x.XunchaAddress,
                        XunchaTime = x.XunchaTime,
