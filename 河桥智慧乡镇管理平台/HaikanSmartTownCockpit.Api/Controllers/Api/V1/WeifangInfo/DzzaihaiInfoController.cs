...
namespace HaikanSmartTownCockpit.Api.Controllers.Api.V1.WeifangInfo
{
    ...
    [ApiController]
    public class DzzaihaiInfoController : ControllerBase
    {
        private readonly haikanHeQiaoContext _dbContext;
        private readonly IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;

        public DzzaihaiInfoController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        public IActionResult GetfoGet(Guid guid)
        {
            using (_dbContext)
            {
                var query1 = from c in _dbContext.DzhizaihaiInfo
                             where c.DzhizaihaiInfoUuid == guid
                             select new
                             {
                                 c.Id,
                                 c.DzhizaihaiInfoUuid,
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE DzhizaihaiInfo SET IsDeleted=@isDeleted WHERE DzhizaihaiInfoUuid IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@isDeleted", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:地质灾害点信息一条数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
        ...
        public IActionResult GetImport(IFormFile excelfile)
        {
            ...
            using (_dbContext)
            {
                ...
                string uploadtitle = " 地质灾害点信息导入" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            var entity = new HaikanSmartTownCockpit.Api.Entities.DzhizaihaiInfo();
                            entity.DzhizaihaiInfoUuid = Guid.NewGuid();
                            if (!string.IsNullOrEmpty(dt.Rows[i]["地区"].ToString()))
                            {
                                entity.Diqu = dt.Rows[i]["地区"].ToString();
                            }
                            else
                            {
                                responsemsgdefault += "<p style='color:red'>" + "第" + (i + 2) + "行地区为空" + "</p></br>";
                                defaultcount++;
                                continue;
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["数量"].ToString()))
                            {
                                entity.Shuliang = dt.Rows[i]["数量"].ToString();
                            }
                    ...
                    ToLog.AddLog("导入", "成功:导入:地质灾害点信息数据", _dbContext);
        ...
        public IActionResult ExportPass(string ids)
        {
            ...
            using (_dbContext)
            {
                if (ids != null)
                {
                    ...
                    var query1 = _dbContext.DzhizaihaiInfo.Where(x => x.IsDeleted != 1 && parameters.Contains(x.DzhizaihaiInfoUuid.ToString())).Select(x => new HaikanSmartTownCockpit.Api.Entities.DzhizaihaiInfo
                    {
                        Id = x.Id,
                        Diqu = x.Diqu,
                        Shuliang = x.Shuliang,
                    ...
                    string sWebRootFolder = _hostingEnvironment.WebRootPath + "\\UploadFiles\\ImportUserInfoExcel\\";
                    string uploadtitle = "地质灾害点信息导出" + DateTime.Now.ToString("yyyyMMddHHmmss");
                    ...
                    ToLog.AddLog("导出", "成功:导出:地质灾害点信息数据", _dbContext);
                    return Ok(response);
                }
                else
                {
                    var query1 = _dbContext.DzhizaihaiInfo.Where(x => x.IsDeleted != 1).Select(x => new HaikanSmartTownCockpit.Api.Entities.DzhizaihaiInfo
                    {
                        Id = x.Id,
                        Diqu = x.Diqu,
                        Shuliang = x.Shuliang,
        ...
        public static bool TablesToExcel(List<HaikanSmartTownCockpit.Api.Entities.DzhizaihaiInfo> query, string filename)
        {
        ...
        private static void CreatSheet(ISheet sheet, List<HaikanSmartTownCockpit.Api.Entities.DzhizaihaiInfo> query)
        {
        ...
            //表头
            ...
            foreach (var row in query)
            {
                ...
                dataRow.CreateCell(0).SetCellValue(row.Diqu);
                dataRow.CreateCell(1).SetCellValue(row.Shuliang);
                rowIndex++;
            }
        }
