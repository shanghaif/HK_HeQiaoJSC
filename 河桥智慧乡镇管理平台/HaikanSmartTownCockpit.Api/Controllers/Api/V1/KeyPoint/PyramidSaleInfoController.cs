...
namespace HaikanSmartTownCockpit.Api.ViewModels.KeyPoint
{
    ...
    [ApiController]
    public class PyramidSaleInfoController : ControllerBase
    {
        private readonly haikanHeQiaoContext _dbContext;
        private readonly IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;

        public PyramidSaleInfoController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        public IActionResult GetfoGet(Guid guid)
        {
            using (_dbContext)
            {
                var query1 = _dbContext.PyramidSale.Where(x => x.PyramidSaleUuid == guid);
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult GetCreate(PyramidSaleViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = new PyramidSale();
                entity.PyramidSaleUuid = Guid.NewGuid();
                ...
                entity.Attention = model.Attention;
                entity.UnlawfulAct = model.UnlawfulAct;
                entity.PenaltyType = model.PenaltyType;
                entity.PyramidSaleType = model.PyramidSaleType;
                entity.PyramidSaleClass = model.PyramidSaleClass;
                entity.BindingHours = model.BindingHours;
                entity.BindingHoursEnd = model.BindingHoursEnd;
                entity.IsDeleted = 0;
                _dbContext.PyramidSale.Add(entity);
                int res = _dbContext.SaveChanges();
                if (res > 0)
                {
                    ToLog.AddLog("添加", "成功:添加:传销信息一条数据", _dbContext);
                }
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult GetEdit(PyramidSaleViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            ...
            using (_dbContext)
            {
                var entity = _dbContext.PyramidSale.FirstOrDefault(x => x.PyramidSaleUuid == model.PyramidSaleUuid);
                entity.Name = model.Name;
                entity.Attention = model.Attention;
                entity.IdCard = model.IdCard;
                entity.OwnedNetwork = model.OwnedNetwork;
                ...
                if (res > 0)
                {
                    ToLog.AddLog("编辑", "成功:编辑:传销信息一条数据", _dbContext);
                }
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE PyramidSale SET IsDeleted=@isDeleted WHERE PyramidSaleUuid IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@isDeleted", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:传销信息一条数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
        ...
        public IActionResult GetImport(IFormFile excelfile)
        {
            ...
            using (_dbContext)
            {
                ...
                string uploadtitle = "传销人员清单导入" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                try
                {
                    ...
                    else
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (!string.IsNullOrEmpty(dt.Rows[i]["传销地址"].ToString()))
                            {
                                entity.OtherAddress = dt.Rows[i]["传销地址"].ToString();
                            }
                            ...
                            if (!string.IsNullOrEmpty(dt.Rows[i]["违法行为"].ToString()))
                            {
                                entity.UnlawfulAct = dt.Rows[i]["违法行为"].ToString();
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["处罚类别"].ToString()))
                            {
                                entity.PenaltyType = dt.Rows[i]["处罚类别"].ToString();
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["传销类别"].ToString()))
                            {
                                entity.PyramidSaleType = dt.Rows[i]["传销类别"].ToString();
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["传销等级"].ToString()))
                            {
                                entity.PyramidSaleClass = dt.Rows[i]["传销等级"].ToString();
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["关注程度"].ToString()))
                            {
                                entity.Attention = dt.Rows[i]["关注程度"].ToString();
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["限制时间"].ToString()))
                            {
                                entity.BindingHours = dt.Rows[i]["限制时间"].ToString();
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["限制结束时间"].ToString()))
                            {
                                entity.BindingHoursEnd = dt.Rows[i]["限制结束时间"].ToString();
                            }
                            entity.IsDeleted = 0;
                            _dbContext.PyramidSale.Add(entity);
                    ...
                    ToLog.AddLog("导入", "成功:导入:传销信息数据", _dbContext);
        ...
        public IActionResult ExportPass(string ids)
        {
            ...
            using (_dbContext)
            {
                if (ids != null)
                {
                    ...
                    var query1 = _dbContext.PyramidSale.Where(x => x.IsDeleted != 1 && parameters.Contains(x.PyramidSaleUuid.ToString())).Select(x => new PyramidSale
                    {
                        Id = x.Id,
                        Name = x.Name,
                        ...
                        UnlawfulAct = x.UnlawfulAct,
                        PenaltyType = x.PenaltyType,
                        PyramidSaleType = x.PyramidSaleType,
                        PyramidSaleClass = x.PyramidSaleClass,
                        BindingHours = x.BindingHours,
                        BindingHoursEnd = x.BindingHoursEnd,
                    });
                    var query = query1.OrderByDescending(x => x.Id).ToList();
                    string sWebRootFolder = _hostingEnvironment.WebRootPath + "\\UploadFiles\\KeyPointExcel\\";
                    string uploadtitle = "传销人员清单导出" + DateTime.Now.ToString("yyyyMMddHHmmss");
                    ...
                    ToLog.AddLog("导出", "成功:导出:传销信息数据", _dbContext);
                    return Ok(response);
                }
                else
                {
                    var query1 = _dbContext.PyramidSale.Where(x => x.IsDeleted != 1).Select(x => new PyramidSale
                    {
                        Id = x.Id,
                        Name = x.Name,
                        OwnedNetwork = x.OwnedNetwork,
        ...
        public static bool TablesToExcel(List<PyramidSale> query, string filename)
        {
        ...
        private static void CreatSheet(ISheet sheet, List<PyramidSale> query)
        {
            IRow headerRow = sheet.CreateRow(0);
            List<string> list = new List<string>() {
  "姓名","所属网格","性别","出生日期","身份证","户籍地址","户籍地详址","户籍派出所","房屋编号","现住地址","无房原因","传销地址","曾用名","工作单位","联系电话","联系手机","家庭成员联系电话","电子邮件","民族","政治面貌","文化程度","职业","婚姻状况","血型","宗教信仰","身高","有无服务成员","最新服务时间","备注","违法行为","处罚类别","传销类别","传销等级","关注程度","限制时间","限制结束时间"
            };
        ...
            //表头
            ...
            foreach (var row in query)
            {
                ...
                dataRow.CreateCell(29).SetCellValue(row.UnlawfulAct);
                dataRow.CreateCell(30).SetCellValue(row.PenaltyType);
                dataRow.CreateCell(31).SetCellValue(row.PyramidSaleType);
                dataRow.CreateCell(32).SetCellValue(row.PyramidSaleClass);
                dataRow.CreateCell(33).SetCellValue(row.Attention);
                dataRow.CreateCell(34).SetCellValue(row.BindingHours);
                dataRow.CreateCell(35).SetCellValue(row.BindingHoursEnd);
                rowIndex++;
            }
        }
