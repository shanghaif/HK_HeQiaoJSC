...
namespace HaikanSmartTownCockpit.Api.Controllers.Api.V1.KeyPoint
{
    ...
    [ApiController]
    public class OtherstaffsInfoController : ControllerBase
    {
        private readonly haikanHeQiaoContext _dbContext;
        private readonly IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;

        public OtherstaffsInfoController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        {
            var query = from c in _dbContext.Otherstaffs
                        where c.IsDeleted == 0
                        select new
                        {
                            c.Id,
            ...
            ToLog.AddLog("导入", "成功:导入:其他人员信息数据", _dbContext);
            return Ok(response);
        }
        ...
        public IActionResult GetfoGet(Guid guid)
        {
            using (_dbContext)
            {
                var query1 = _dbContext.Otherstaffs.Where(x => x.OtherstaffsUuid == guid);
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult GetCreate(OtherstaffsViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = new Otherstaffs();
                entity.OtherstaffsUuid = Guid.NewGuid();
                ...
                entity.IsDeleted = 0;
                _dbContext.Otherstaffs.Add(entity);
                int res = _dbContext.SaveChanges();
                if (res > 0)
                {
                    ToLog.AddLog("添加", "成功:添加:其他人员信息一条数据", _dbContext);
                }
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult GetEdit(OtherstaffsViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            ...
            using (_dbContext)
            {
                var entity = _dbContext.Otherstaffs.FirstOrDefault(x => x.OtherstaffsUuid == model.OtherstaffsUuid);
                entity.Name = model.Name;
                entity.Attention = model.Attention;
                entity.IdCard = model.IdCard;
                entity.OwnedNetwork = model.OwnedNetwork;
                ...
                if (res > 0)
                {
                    ToLog.AddLog("编辑", "成功:编辑:其他人员信息一条数据", _dbContext);
                }
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE Otherstaffs SET IsDeleted=@isDeleted WHERE OtherstaffsUuid IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@isDeleted", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:其他人员信息一条数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
        ...
        public IActionResult GetImport(IFormFile excelfile)
        {
            ...
            using (_dbContext)
            {
                ...
                string uploadtitle = "其他人员清单导入" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                try
                {
                    ...
                    else
                    {
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (!string.IsNullOrEmpty(dt.Rows[i]["工作情况"].ToString()))
                            {
                                entity.WorkCondition = dt.Rows[i]["工作情况"].ToString();
                            }
                            entity.IsDeleted = 0;
                            _dbContext.Otherstaffs.Add(entity);
        ...
        public IActionResult ExportPass(string ids)
        {
            ...
            using (_dbContext)
            {
                if (ids != null)
                {
                    ...
                    var query1 = _dbContext.Otherstaffs.Where(x => x.IsDeleted != 1 && parameters.Contains(x.OtherstaffsUuid.ToString())).Select(x => new Otherstaffs
                    {
                        Id = x.Id,
                        Name = x.Name,
                        ...
                        AttentionCause = x.AttentionCause,
                        WorkCondition = x.WorkCondition,
                    });
                    var query = query1.OrderByDescending(x => x.Id).ToList();
                    string sWebRootFolder = _hostingEnvironment.WebRootPath + "\\UploadFiles\\KeyPointExcel\\";
                    string uploadtitle = "其他人员清单导出" + DateTime.Now.ToString("yyyyMMddHHmmss");
                    ...
                    ToLog.AddLog("导出", "成功:导出:其他人员信息数据", _dbContext);
                    return Ok(response);
                }
                else
                {
                    var query1 = _dbContext.Otherstaffs.Where(x => x.IsDeleted != 1).Select(x => new Otherstaffs
                    {
                        Id = x.Id,
                        Name = x.Name,
                        OwnedNetwork = x.OwnedNetwork,
        ...
        public static bool TablesToExcel(List<Otherstaffs> query, string filename)
        {
        ...
        private static void CreatSheet(ISheet sheet, List<Otherstaffs> query)
        {
            IRow headerRow = sheet.CreateRow(0);
            List<string> list = new List<string>() {
  "姓名","所属网格","性别","出生日期","身份证","户籍地址","户籍地详址","户籍派出所","房屋编号","现住地址","无房原因","其他地址","曾用名","工作单位","联系电话","联系手机","家庭成员联系电话","电子邮件","民族","政治面貌","文化程度","职业","婚姻状况","血型","宗教信仰","身高","有无服务成员","最新服务时间","备注","关注程度","关注原因","工作情况"
            };
        ...
            //表头
            ...
            foreach (var row in query)
            {
                ...
                dataRow.CreateCell(29).SetCellValue(row.Attention);
                dataRow.CreateCell(30).SetCellValue(row.AttentionCause);
                dataRow.CreateCell(31).SetCellValue(row.WorkCondition);
                rowIndex++;
            }
        }
