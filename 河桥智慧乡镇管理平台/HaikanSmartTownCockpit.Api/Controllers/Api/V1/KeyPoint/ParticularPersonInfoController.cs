...
namespace HaikanSmartTownCockpit.Api.Controllers.Api.V1.KeyPoint
{
    ...
    [ApiController]
    public class ParticularPersonInfoController : ControllerBase
    {
        private readonly haikanHeQiaoContext _dbContext;
        private readonly IMapper _mapper;
        private IWebHostEnvironment _hostingEnvironment;

        public ParticularPersonInfoController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        {
            var query = from c in _dbContext.ParticularPerson
                        where c.IsDeleted == 0
                        select new
                        {
                            c.Id,
                            c.ParticularPersonUuid,
                            c.Name,
                            c.OwnedNetwork,
                            c.Type,      //类别
                            c.AttentionCause,      //关注原因
                            c.Attention,        //关注程度
                            c.IdCard,
                        };
            ...
            ToLog.AddLog("查询", "成功:查询:特定人员信息数据", _dbContext);
            return Ok(response);
        }
        ...
        public IActionResult GetfoGet(Guid guid)
        {
            using (_dbContext)
            {
                var query1 = _dbContext.ParticularPerson.Where(x => x.ParticularPersonUuid == guid);
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult GetEdit(ParticularPersonViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            ...
            using (_dbContext)
            {
                var entity = _dbContext.ParticularPerson.FirstOrDefault(x => x.ParticularPersonUuid == model.ParticularPersonUuid);
                               entity.Name = model.Name;
                entity.Attention = model.Attention;
                entity.IdCard = model.IdCard;
                entity.OwnedNetwork = model.OwnedNetwork;
                ...
                if (res > 0)
                {
                    ToLog.AddLog("编辑", "成功:编辑:特定人员信息一条数据", _dbContext);
                }
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE ParticularPerson SET IsDeleted=@isDeleted WHERE ParticularPersonUuid IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@isDeleted", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:特定人员信息一条数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
        ...
        public IActionResult ExportPass(string ids)
        {
            ...
            using (_dbContext)
            {
                if (ids != null)
                {
                    ...
                    var query1 = _dbContext.ParticularPerson.Where(x => x.IsDeleted != 1 && parameters.Contains(x.ParticularPersonUuid.ToString())).Select(x => new ParticularPerson
                    {
                        Id = x.Id,
                        Name = x.Name,
                        ...
                        Privy = x.Privy,
                        PrivyPhone = x.PrivyPhone,
                    });
                    var query = query1.OrderByDescending(x => x.Id).ToList();
                    string sWebRootFolder = _hostingEnvironment.WebRootPath + "\\UploadFiles\\KeyPointExcel\\";
                    string uploadtitle = "特定人员清单导出" + DateTime.Now.ToString("yyyyMMddHHmmss");
                    ...
                    ToLog.AddLog("导出", "成功:导出:特定人员信息数据", _dbContext);
                    return Ok(response);
                }
                else
                {
                    var query1 = _dbContext.ParticularPerson.Where(x => x.IsDeleted != 1).Select(x => new ParticularPerson
                    {
                        Id = x.Id,
                        Name = x.Name,
                        OwnedNetwork = x.OwnedNetwork,
        ...
        public static bool TablesToExcel(List<ParticularPerson> query, string filename)
        {
        ...
        private static void CreatSheet(ISheet sheet, List<ParticularPerson> query)
        {
            IRow headerRow = sheet.CreateRow(0);
            List<string> list = new List<string>() {
  "姓名","所属网格","性别","出生日期","身份证","户籍地址","户籍地详址","户籍派出所","房屋编号","现住地址","无房原因","特定地址","曾用名","工作单位","联系电话","联系手机","家庭成员联系电话","电子邮件","民族","政治面貌","文化程度","职业","婚姻状况","血型","宗教信仰","身高","有无服务成员","最新服务时间","备注","类别","加入时间","关注程度","关注原因","关系人","关系人电话"
            };
        ...
            //表头
            ...
            foreach (var row in query)
            {
                ...
                dataRow.CreateCell(29).SetCellValue(row.Type);
                dataRow.CreateCell(30).SetCellValue(row.JoinTime);
                dataRow.CreateCell(31).SetCellValue(row.Attention);
                dataRow.CreateCell(32).SetCellValue(row.AttentionCause);
                dataRow.CreateCell(33).SetCellValue(row.Privy);
                dataRow.CreateCell(34).SetCellValue(row.PrivyPhone);
                rowIndex++;
            }
        }
