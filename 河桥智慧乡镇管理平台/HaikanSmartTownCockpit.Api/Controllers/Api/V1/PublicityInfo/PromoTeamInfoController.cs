...
namespace HaikanSmartTownCockpit.Api.Controllers.Api.V1.PublicityInfo
{
    ...
    [ApiController]
    public class PromoTeamInfoController : ControllerBase
    {
        ...
        public PromoTeamInfoController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        /// <returns></returns>
        [HttpGet("{guid}")]
        public IActionResult EditGet(Guid guid)
        {
            using (_dbContext)
            {
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted IsDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE PromoTeam SET IsDeleted=@IsDeleted WHERE PromoTeamUuid IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@IsDeleted", (int)IsDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:党建宣传小队信息一条数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
        ...
        public IActionResult zhnedilx()
        {
            ...
            using (_dbContext)
            {
                var query = from p in _dbContext.Propaganda
                            where p.IsDeleted == 0
                            select new
                            {
                                Value = p.PropagandaTypeName,
                                Label = p.PropagandaTypeName,
                            };
