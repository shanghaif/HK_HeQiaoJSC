...
namespace HaikanSmartTownCockpit.Api.Controllers.Api.V1.Organization
{
    ...
    [ApiController]
    public class MilitiaController : ControllerBase
    {
        ...
        public MilitiaController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        {
            var query = from c in _dbContext.Cunzhenmb
                        where c.IsDeleted == 0
                        select new
                        {
                            c.CunzhenmbUuid,
            ...
            ToLog.AddLog("查询", "成功:查询:村镇民兵信息数据", _dbContext);
            return Ok(response);
        }
        ...
        public IActionResult Show(Guid guid)
        {
            ...
            using (_dbContext)
            {
                var entity = _dbContext.Cunzhenmb.FirstOrDefault(x => x.CunzhenmbUuid == guid);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                var sql = string.Format("UPDATE Cunzhenmb SET IsDeleted=@isDeleted WHERE CunzhenmbUuid IN ({0})", parameterNames);
                parameters.Add(new SqlParameter("@isDeleted", (int)isDeleted));
                _dbContext.Database.ExecuteSqlRaw(sql, parameters);
                ToLog.AddLog("删除", "成功:删除:村镇民兵信息一条数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
        ...
        public IActionResult Import(IFormFile excelfile)
        {
            ...
            using (_dbContext)
            {
                ...
                string uploadtitle = "村镇民兵信息导入" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            var entity = new HaikanSmartTownCockpit.Api.Entities.Cunzhenmb();
                            entity.CunzhenmbUuid = Guid.NewGuid();
                            ...
                            {
                                entity.Residence = dt.Rows[i]["职务"].ToString();
                            }
                            ...
                            {
                                entity.Nation = dt.Rows[i]["政治面貌"].ToString();
                            }
                            ...
                            if (!string.IsNullOrEmpty(dt.Rows[i]["工作单位或家庭地址"].ToString()))
                            {
                                entity.Address = dt.Rows[i]["工作单位或家庭地址"].ToString();
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["班级"].ToString()))
                            {
                                entity.Wechat = dt.Rows[i]["班级"].ToString();
                            }
                            entity.AddTime = DateTime.Now.ToString("yyyy-MM-dd");
                            entity.AddPeople = AuthContextService.CurrentUser.DisplayName;
                            entity.IsDeleted = 0;
                    ...
                    ToLog.AddLog("导入", "成功:导入:村镇民兵信息一条数据", _dbContext);
        ...
        public IActionResult ExportPass(string ids)
        {
            ...
            using (_dbContext)
            {
                if (ids != null)
                {
                    ...
                    var query = _dbContext.Cunzhenmb.Where(x => x.IsDeleted != 1 && parameters.Contains(x.CunzhenmbUuid.ToString())).Select(x => new HaikanSmartTownCockpit.Api.Entities.Cunzhenmb
                    {
                        RealName = x.RealName,
                        Address = x.Address,
                        Sex = x.Sex,
                    ...
                    string sWebRootFolder = _hostingEnvironment.WebRootPath + "\\UploadFiles\\ImportBuidingExcel\\";
                    string uploadtitle = "村镇民兵信息导出" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                {
                    var query = _dbContext.Cunzhenmb.Where(x => x.IsDeleted != 1).Select(x => new HaikanSmartTownCockpit.Api.Entities.Cunzhenmb
                    {
                        RealName = x.RealName,
                        Address = x.Address,
                        Sex = x.Sex,
        ...
        public static bool TablesToExcel(List<HaikanSmartTownCockpit.Api.Entities.Cunzhenmb> query, string filename)
        {
            ...
            ISheet sheet = workBook.CreateSheet("村镇民兵表");
