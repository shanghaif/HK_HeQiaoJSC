...
namespace HaikanSmartTownCockpit.Api.Controllers.Api.V1.Organization
{
    ...
    [ApiController]
    public class OldmanController : ControllerBase
    {
        ...
        public OldmanController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        public IActionResult Create(dynamic model)
        {
            ...
            using (_dbContext)
            {
                ...
                entity.TutelageName = model.tutelageName;
                entity.TutelagePhone = model.tutelagePhone;
                entity.HouseAddress = model.houseAddress;
                entity.Specific = model.specific;
                entity.ManType = "孤寡老人";
        ...
        public IActionResult Import(IFormFile excelfile)
        {
            ...
            using (_dbContext)
            {
                ...
                string uploadtitle = "孤寡老人信息导入" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (!dt.Columns.Contains("监护人"))
                        {
                            response.SetFailed("无‘监护人’列");
                            return Ok(response);
                        }
                        if (!dt.Columns.Contains("监护人联系方式"))
                        {
                            response.SetFailed("无‘监护人联系方式’列");
                            return Ok(response);
                        }
                        if (!dt.Columns.Contains("家庭地址"))
                        {
                            response.SetFailed("无‘家庭地址’列");
                            return Ok(response);
                        }
                        if (!dt.Columns.Contains("具体情况"))
                        {
                            response.SetFailed("无‘具体情况’列");
                            return Ok(response);
                        }
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (!string.IsNullOrEmpty(dt.Rows[i]["监护人"].ToString()))
                            {
                                Regex reg = new Regex("^([\\u4e00-\\u9fa5]){2,7}$");
                                if (reg.IsMatch(dt.Rows[i]["监护人"].ToString()))
                                {
                                    entity.ZjWork = dt.Rows[i]["监护人"].ToString();
                                }
                                else
                                {
                                    responsemsgdefault += "<p style='color:red'>" + "第" + (i + 2) + "行监护人格式不正确" + "</p></br>";
                                    defaultcount++;
                                    continue;
                                }
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["监护人联系方式"].ToString()))
                            {
                                Regex reg = new Regex("^[1][3,4,5,7,8][0-9]{9}$");
                                if (reg.IsMatch(dt.Rows[i]["监护人联系方式"].ToString()))
                                {
                                    entity.Duty = dt.Rows[i]["监护人联系方式"].ToString();
                                }
                                else
                                {
                                    responsemsgdefault += "<p style='color:red'>" + "第" + (i + 2) + "行监护人联系方式格式不正确" + "</p></br>";
                                    defaultcount++;
                                    continue;
                                }
                            }
                            ...
                            {
                                entity.HouseAddress = dt.Rows[i]["家庭地址"].ToString();
                            }
                            if (!string.IsNullOrEmpty(dt.Rows[i]["具体情况"].ToString()))
                            {
                                entity.Specific = dt.Rows[i]["具体情况"].ToString();
                            }
                            entity.OrganizationUuid = query.OrganizationUuid;
                            entity.ManType = "孤寡老人";
                            entity.AddTime = DateTime.Now.ToString("yyyy-MM-dd");
            ...
            using (_dbContext)
            {
                if (ids != null)
                {
                    ...
                    var query = _dbContext.OrganPeoInfo.Where(x => x.IsDeleted != 1 && parameters.Contains(x.OrganPeoInfoUuid.ToString())).Select(x => new HaikanSmartTownCockpit.Api.Entities.OrganPeoInfo
                    {
                        ...
                        Sex = x.Sex,
                        Birth = x.Birth,
                        IdentityCard = x.TutelageName,
                        Phone = x.TutelagePhone,
                        HouseAddress = x.HouseAddress,
                        Specific = x.Specific
                    }).ToList();
                    string sWebRootFolder = _hostingEnvironment.WebRootPath + "\\UploadFiles\\ImportBuidingExcel\\";
                    string uploadtitle = "孤寡老人信息导出" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                else
                {
                    ...
                    var query = _dbContext.OrganPeoInfo.Where(x => x.IsDeleted != 1 && x.OrganizationUuid == oquery.OrganizationUuid && x.ManType == "孤寡老人").Select(x => new HaikanSmartTownCockpit.Api.Entities.OrganPeoInfo
                    {
                        OrganName = x.OrganName,
                        Sex = x.Sex,
                        Birth = x.Birth,
        ...
        public static bool TablesToExcel(List<HaikanSmartTownCockpit.Api.Entities.OrganPeoInfo> query, string filename)
        {
            ...
            ISheet sheet = workBook.CreateSheet("孤寡老人信息表");
        ...
        private static void CreatSheet(ISheet sheet, List<HaikanSmartTownCockpit.Api.Entities.OrganPeoInfo> query)
        {
            ...
            List<string> list = new List<string>() {
                "姓名","性别","出生日期","监护人","监护人联系方式","家庭地址","具体情况"
            };
        ...
            //表头
            ...
            foreach (var row in query)
            {
                ...
                dataRow.CreateCell(3).SetCellValue(row.TutelageName);
                dataRow.CreateCell(4).SetCellValue(row.TutelagePhone);
                dataRow.CreateCell(5).SetCellValue(row.HouseAddress);
                dataRow.CreateCell(6).SetCellValue(row.Specific);
                rowIndex++;
            }
        }
