...
namespace HaikanSmartTownCockpit.Api.Controllers.Api.V1.Organization
{
    ...
    [ApiController]
    public class GQTInfoController : ControllerBase
    {
        ...
        public GQTInfoController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(GQTInfoRequestpayload payload)
        {
            var query = from t in _dbContext.OrganPeoInfo
                        ...
                        where t.IsDeleted != 1 && s.OrganizationName=="共青团"
                        select new
                        {
                            t.Id,
                            t.OrganName,
        ...
        public IActionResult ExportPass(string ids)
        {
            ...
            using (_dbContext)
            {
                if (ids != null)
                {
                    ...
                    string sWebRootFolder = _hostingEnvironment.WebRootPath + "\\UploadFiles\\ImportBuidingExcel\\";
                    string uploadtitle = "共青团信息导出" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                {
                    var oquery = _dbContext.Organization.FirstOrDefault(x=>x.OrganizationName=="共青团");
        ...
        public static bool TablesToExcel(List<HaikanSmartTownCockpit.Api.Entities.OrganPeoInfo> query, string filename)
        {
            ...
            ISheet sheet = workBook.CreateSheet("共青团表");
