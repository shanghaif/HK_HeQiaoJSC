...
namespace HaikanSmartTownCockpit.Api.Controllers.Api.V1.Organization
{
    ...
    [ApiController]
    public class ZGHInfoController : ControllerBase
    {
        ...
        public ZGHInfoController(haikanHeQiaoContext dbContext, IMapper mapper, IWebHostEnvironment hostingEnvironment)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(ZGHInfoRequestpayload payload)
        {
            var query = from t in _dbContext.OrganPeoInfo
                        ...
                        where t.IsDeleted != 1 && s.OrganizationName == "总工会"
                        select new
                        {
                            t.Id,
                            t.OrganName,
        ...
        public IActionResult Create(dynamic model)
        {
            ...
            using (_dbContext)
            {
                var query = _dbContext.Organization.FirstOrDefault(x => x.OrganizationName == "总工会");
        ...
        public IActionResult Import(IFormFile excelfile)
        {
            ...
            using (_dbContext)
            {
                ...
                string uploadtitle = "组织信息导入" + DateTime.Now.ToString("yyyyMMddHHmmss");
        ...
        public IActionResult ExportPass(string ids)
        {
            ...
            using (_dbContext)
            {
                if (ids != null)
                {
                    ...
                    string sWebRootFolder = _hostingEnvironment.WebRootPath + "\\UploadFiles\\ImportBuidingExcel\\";
                    string uploadtitle = "总工会信息导出" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ...
                {
                    var oquery = _dbContext.Organization.FirstOrDefault(x => x.OrganizationName == "总工会");
                    var query = _dbContext.OrganPeoInfo.Where(x => x.IsDeleted != 1 && x.OrganizationUuid == oquery.OrganizationUuid).Select(x => new HaikanSmartTownCockpit.Api.Entities.OrganPeoInfo
                    {
                        OrganName = x.OrganName,
                        Sex = x.Sex,
                        Birth = x.Birth,
        ...
        public static bool TablesToExcel(List<HaikanSmartTownCockpit.Api.Entities.OrganPeoInfo> query, string filename)
        {
            ...
            ISheet sheet = workBook.CreateSheet("总工会信息表");
