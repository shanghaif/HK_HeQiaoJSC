    ...
    public class GtoiletController : ControllerBase
    {
        ...
        public IActionResult List(GtoiletRequestPayload payload)
        {
            using (_dbContext)
            {
                ...
                ToLog.AddLog("查询", "成功:查询:3A公厕信息数据", _dbContext);
                return Ok(response);
            }
        }
        #endregion
        ...
        public IActionResult Edit(Guid guid)
        {
            using (_dbContext)
            {
                ...
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map<Gtoilet, GtoiletViewModel>(entity));
                return Ok(response);
            }
        }
        /// <summary>
        ...
        public IActionResult Edit(GtoiletViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                if (res > 0)
                {
                    ToLog.AddLog("编辑", "成功:编辑:3A公厕信息一条数据", _dbContext);
                }
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                ...
                ToLog.AddLog("删除", "成功:删除:3A公厕信息一条数据", _dbContext);
                var response = ResponseModelFactory.CreateInstance;
                return response;
            }
        }
        ...
        public IActionResult GtoiletImport(IFormFile excelfile)
        {
            ...
            using (_dbContext)
            {
                ...
                try
                {
                    ...
                    ToLog.AddLog("导入", "成功:导入:3A公厕信息数据", _dbContext);
