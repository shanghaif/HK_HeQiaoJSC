                ...
                <Form inline @submit.native.prevent>
                  <FormItem>
                    <Input
                      ...
                      search
                      :clearable="true"
                      v-model="stores.demo.query.name"
                      placeholder="请输入落户人员姓名"
                  ...
                  <FormItem>
                    <Input
                      ...
                      search
                      :clearable="true"
                      v-model="stores.demo.query.idCard"
                      placeholder="请输入落户人员身份证号"
                      @on-search="handleSearchDispatch()"
                    ></Input>
                  </FormItem>
                  <FormItem>
                    <DatePicker
                    v-model="stores.demo.query.sTime"
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="出生日期" prop="birthdate">
            <DatePicker
              v-model="formModel.fields.birthdate"
              @on-change="formModel.fields.birthdate = $event"
              format="yyyy-MM-dd"
              type="date"
              placeholder="请选择出生日期"
              style="width: 100%"
        ...
        <Row :gutter="16">
          <FormItem label="籍贯" prop="nativePlace">
            <Input
              v-model="formModel.fields.nativePlace"
              :readonly="checkShow()"
              placeholder="请输入籍贯"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="居住地" prop="placeAbode">
            <Input
              v-model="formModel.fields.placeAbode"
              :readonly="checkShow()"
              placeholder="请输入居住地"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="落户时间" prop="settleTime">
            <DatePicker
              v-model="formModel.fields.settleTime"
              @on-change="formModel.fields.settleTime = $event"
              format="yyyy-MM-dd"
              type="date"
              placeholder="请选择落户时间"
              style="width: 100%"
        ...
        <Row :gutter="16">
          <FormItem label="迁移地" prop="qianYiDe">
            <Input
              v-model="formModel.fields.qianYiDe"
              :readonly="checkShow()"
              placeholder="请输入迁移地"
            />
          </FormItem>
...
import {
  ...
  GetImport, //导入
  GetExportPass, //导出
} from "@/api/SettleInfo/settleInfo";
export default {
  name: "settleInfo",
  components: {
    DzTable,
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "settleUuid" },
            { title: "姓名", key: "name", ellipsis: true, tooltip: true },
            { title: "性别", key: "sex" },
            {
              title: "出生日期",
              key: "birthdate",
      ...
      formModel: {
        ...
        rules: {
          ...
          placeAbode: [
            { type: "string", required: true, message: "请输入居住地" },
            { validator: globalvalidateIsNotEmpty, trigger: "blur" },
          ]
        },
      },
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.settleUuid);
    }, //删除
  },
  methods: {
    //#region  页面加载
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.settleUuid);
    },
    //详情显示
    handleDetail(e) {
    ...
    doEditDispatch() {
      if (
        ...
        this.formModel.fields.activityTime = new Date(
          Date.parse(new Date(this.formModel.fields.activityTime)) +
            8 * 3600 * 1000
        );
      }
    ...
    handleimportmodel() {
      window.location.href =
        this.url + "UploadFiles/ImportBuidingModel/落户人员信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
  ...
  created() {
    ...
    this.actionurl = config.baseUrl.dev + "api/v1/SettleInfo/SettleInfo/UpLoad";
  },
};
</script>