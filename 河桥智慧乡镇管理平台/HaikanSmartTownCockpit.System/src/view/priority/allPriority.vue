      ...
      <Form :model="formModel.fields" ref="formRole" :rules="formModel.rules" label-position="left">
        ...
        <FormItem label="负责人" prop="principal">
          <Input
            ...
            :readonly="true"
          />
          <i-button type="info" @click="selectfuze">选择负责人</i-button>
        </FormItem>
    ...
    return {
      ...
      stores: {
        ...
        columns: [
          ...
          { title: "完成进度", ellipsis: true, key: "unfinished" },

          //{ title: "排序字段", ellipsis: true, key: "sortord" },
...
handleSearch () {
    ...
    rowClassName(row, index) {
      if (row.accomplish === "2") {
        if (row.establishTime !== row.zhidingshijian) {
    ...
    selectfuze() {
      ...
      //this.formModelfuze.fields.participantright=[];
    ...
    handleSchoolChangefuze(newTargetKeys, direction, moveKeys) {
      ...
      this.formModel.fields.principalname = this.formModel.fields.principalname =
        ""; //每次选择则负责人都重新赋值       
      for (
        let i = 0;
    ...
    handleSchoolChange(newTargetKeys, direction, moveKeys) {
      ...
      this.formModel.fields.participant = this.formModel.fields.participant =
        ""; //每次选择则参与人都重新赋值
      // console.log(this.formModel.fields.participant)
      for (
        let i = 0;
