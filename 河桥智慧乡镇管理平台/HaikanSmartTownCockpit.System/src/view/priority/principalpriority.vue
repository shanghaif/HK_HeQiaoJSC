  ...
  <div>
      ...
      >
        ...
        >
          ...
          <template slot-scope="{ row, index }" slot="action">
            ...
            <Tooltip placement="top" content="完结" :delay="1000" v-if="wanjie(row)" :transfer="true">
              <Button
                v-can="'accomplish'"
                type="warning"
                size="small"
                shape="circle"
                icon="md-checkmark"
                @click="handleshows(row)"
              ></Button>
            </Tooltip>
            <Tooltip placement="top" content="发送工作通知" :delay="1000" v-if="tongzhishow(row)" :transfer="true">
							<Button v-can="'sendout'" type="info" size="small" shape="circle" icon="md-log-in" @click="tongzhi(row)"></Button>
						</Tooltip>
          </template>
        </Table>
      </dz-table>
    ...
    >
      <Form :model="formModel.fields" ref="formRole" :rules="formModel.rules" label-position="left">
        ...
        <FormItem label="结束时间" prop="endTime">
						<Date-picker type="datetime" format="yyyy-MM-dd HH:mm" placeholder="请选择时间" @on-change="formModel.fields.endTime=$event"
						 :value="formModel.fields.endTime"></Date-picker>
					</FormItem>
    ...
    >
      <Form :model="formMode2.fields" ref="详情" label-position="left">
        ...
        <FormItem label="备注">
          <Input v-model="formMode2.fields.remark" :readonly="beizhu" placeholder="请输入备注" />
        </FormItem>
      </Form>
      <div class="demo-drawer-footer">
        <Button
          ...
          type="primary"
          v-if="wanjieshow"
          @click="dopriorityAccomplishRole"
        >完 结</Button>
        <Button
    ...
    return {
      ...
      stores: {
        ...
        columns: [
          ...
          { title: "负责人", ellipsis: true, tooltip: true, key: "principal" },
...
handleSearch () {
    ...
    //查询数据列表
    loadRoleList() {
      principalListRole(this.stores.role.query).then((res) => {
    ...
    handleshows(row) {
      this.wanjieshow = true;
      this.beizhu = false;
      this.handleSwitchFormModeToShow();
      this.handleResetFormRole();
      this.doLoadShow(row.id);
    },
			...
			if(this.formModelfuze.fields.fuzeright.length==0)
			{
					...
					if(res.data.data.length>0)
					{
					    for (let i = 0; i < res.data.data.length; i++) {
							//显示数据库保存的人员
					        this.formModelfuze.fields.participantright = this.formModelfuze.fields.participantright.concat(res.data.data[i].key);
					    }
					}			
			}
    ...
    handleSchoolChangefuze(newTargetKeys, direction, moveKeys) {
      ...
      this.formModel.fields.principal = this.formModel.fields.principal = ""; //每次选择则负责人都重新赋值
      for (
        let i = 0;
        ...
        for (
          ...
          if (
            ...
            this.formModel.fields.principal +=this.formModelfuze.fields.participantleft[j].key + ","; //id
    ...
    doLoadRole(id) {
      loadRole({ id: id }).then((res) => {
        ...
        //this.formModel.fields.principal=res.data.data.principalname;
      });
    },
			...
			tongzhishow(row){
				var shows=false;
				if(row.principaluuids.search(this.$store.state.user.userGuid)!=-1)
				{
					shows=true;
				}
				...
				var show = false;
				if (row.Accomplish != "2"&&shows) //进行中的任务可以发送消息推送
				{
					show = true;
				}
				return show
			...
			//发送消息
			tongzhi(row){
				notescanyu({uuid:row.priorityUuid}).then(res => {
							if (res.data.code === 200) {
								this.$Message.success(res.data.message);
							} else {
								this.$Message.warning(res.data.message);
