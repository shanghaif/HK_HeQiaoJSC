...
import {
  ...
  batchCommand,
  deletePromoInfo
} from "@/api/DangJibenInfo/DangActivityInfo";
...
export default {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.dangActivityUuid);
    }
  },
  methods: {
    ...
    //单条删除
    handleDelete(row) {
      this.doDelete(row.dangActivityUuid);
    },
    doDelete(ids) {
      if (!ids) {
        this.$Message.warning("请选择至少一条数据");
    ...
    handleEdit(row) {
      ...
      this.doLoadPromoInfo(row.dangActivityUuid);
    },
    handleShow(row) {
