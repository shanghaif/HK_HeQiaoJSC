  ...
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="出租房信息" prop="rentoutInfo">
              <Input
                v-model="formModel.fields.rentoutInfo"
                placeholder="请输入出租房信息"
                style="width: 400px"
              />
            </FormItem>
          </Col>
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="业主" prop="rentoutYezhu">
              <Input
                v-model="formModel.fields.rentoutYezhu"
                placeholder="请输入业主"
                style="width: 400px"
              />
            </FormItem>
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="租户" prop="rentoutZuhu">
              <Input
                v-model="formModel.fields.rentoutZuhu"
                placeholder="请输入租户"
                style="width: 400px"
              />
            </FormItem>
        ...
        <Row :gutter="16">
          <Col span="16">
            <FormItem label="租房时间" prop="rentoutTime">
              <DatePicker
                v-model="formModel.fields.rentoutTime"
                @on-change="formModel.fields.rentoutTime=$event"
                format="yyyy-MM-dd"
                type="date"
                placeholder="请输入租房时间"
                style="width: 400px"
        ...
        <Row :gutter="16">
          <Col span="16">
            <FormItem label="到期时间" prop="daoqiTime">
              <DatePicker
                v-model="formModel.fields.daoqiTime"
                @on-change="formModel.fields.daoqiTime=$event"
                format="yyyy-MM-dd"
                type="date"
                placeholder="请输入到期时间"
                style="width: 400px"
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="价格" prop="rentoutMoney">
              <Input
                v-model="formModel.fields.rentoutMoney"
                placeholder="请输入价格"
                style="width: 400px"
                :maxlength="20"
              />
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="出租状态" prop="rentoutStaues">
              <Select
                filterable
                v-model="formModel.fields.rentoutStaues"
                style="width:180px"
                placeholder="出租状态"
              >
                <Option
                  v-for="item in stores.rescueteam.sources.sexList"
                  :value="item.value"
                  :key="item.value"
                >{{ item.label }}</Option>
              </Select>
...
import {
  ...
  HqCommunaImport, //导入
  HqCommunaExport //导出
} from "@/api/Socialgovernance/rentalHous";
...
export default {
      ...
      stores: {
        rescueteam: {
          ...
          columns: [
            ...
            { title: "出租房信息", key: "rentoutInfo", sortable: true },
            { title: "业主", key: "rentoutYezhu", sortable: true },
            { title: "租户", key: "rentoutZuhu", sortable: true },
            { title: "租房时间", key: "rentoutTime", sortable: true },
            { title: "到期时间", key: "daoqiTime", sortable: true },
            { title: "价格", key: "rentoutMoney", sortable: true },
            { title: "出租状态", key: "rentoutStaues", sortable: true },
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.rentoutRoomUuid);
    }
  },
  methods: {
    loadRescueteamList() {
    ...
    //单条删除
    handleDelete(row) {
      this.doDelete(row.rentoutRoomUuid);
    },
    doDelete(ids) {
      if (!ids) {
        this.$Message.warning("请选择至少一条数据");
    ...
    handleEdit(row) {
      ...
      this.doLoadRescueteam(row.rentoutRoomUuid);
    },
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportSocialgoverModel/出租房信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
