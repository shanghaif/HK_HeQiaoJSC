<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="姓名" prop="administratorName">
            <Input v-model="formModel.fields.administratorName" placeholder="请输入姓名" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="身份证号" prop="identityCard">
            <Input v-model="formModel.fields.identityCard" placeholder="请输入身份证号" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="手机号码" prop="phone">
            <Input v-model="formModel.fields.phone" placeholder="请输入手机号码" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="行政村" prop="adminVillages">
            <Input v-model="formModel.fields.adminVillages" placeholder="请输入行政村" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="所在网格" prop="suozaiWangge">
            <Input v-model="formModel.fields.suozaiWangge" placeholder="请输入所在网格" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="网格账号" prop="wanggeZhanghao">
            <Input v-model="formModel.fields.wanggeZhanghao" placeholder="请输入网格账号" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="村级账号" prop="cunjiZhanghao">
            <Input v-model="formModel.fields.cunjiZhanghao" placeholder="请输入村级账号" />
          </FormItem>
        </Row>
      </Form>
      <div>
...
import {
  ...
  HqCommunaImport, //导入
  HqCommunaExport //导出
} from "@/api/Socialgovernance/administrator";
...
export default {
    ...
    return {
      ...
      formModel: {
        ...
        rules: {
          ...
          adminVillages: [
            { required: true, message: "请输入行政村", trigger: "blur" }
          ]
        }
      },
      stores: {
        rescueteam: {
          ...
          columns: [
            ...
            { title: "姓名", key: "administratorName", sortable: true },
            { title: "手机号码", key: "phone", sortable: true },
            { title: "行政村", key: "adminVillages", sortable: true },
            { title: "所在网格", key: "suozaiWangge", sortable: true },
            {
              title: "操作",
              fixed: "right",
              align: "center",
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.administratorUuid);
    }
  },
  methods: {
    loadRescueteamList() {
    ...
    //单条删除
    handleDelete(row) {
      this.doDelete(row.administratorUuid);
    },
    doDelete(ids) {
      if (!ids) {
        this.$Message.warning("请选择至少一条数据");
    ...
    handleEdit(row) {
      ...
      this.doLoadRescueteam(row.administratorUuid);
    },
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url +
        "UploadFiles/ImportUserInfoModel/网格管理员信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
