<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="企业名称" prop="qiyeName">
            <Input v-model="formModel.fields.qiyeName" placeholder="企业名称" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="企业地址" prop="qiyeAddress">
            <Input v-model="formModel.fields.qiyeAddress" placeholder="企业地址" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="法人" prop="faren">
            <Input v-model="formModel.fields.faren" placeholder="法人" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="联系方式" prop="phone">
            <Input v-model="formModel.fields.phone" placeholder="联系方式" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="规模" prop="guimo">
            <Input v-model="formModel.fields.guimo" placeholder="规模" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="企业类型" prop="qiyeType">
            <Select
              filterable
              v-model="formModel.fields.qiyeType"
              style="width:180px"
              placeholder="企业类型"
            >
              <Option
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="企业名称" prop="qiyeName">
            <Input v-model="formModel1.fields.qiyeName" placeholder="企业名称" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="企业地址" prop="qiyeAddress">
            <Input v-model="formModel1.fields.qiyeAddress" placeholder="企业地址" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="法人" prop="faren">
            <Input v-model="formModel1.fields.faren" placeholder="法人" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="联系方式" prop="phone">
            <Input v-model="formModel1.fields.phone" placeholder="联系方式" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="规模" prop="guimo">
            <Input v-model="formModel1.fields.guimo" placeholder="规模" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="企业类型" prop="qiyeType">
            <Input v-model="formModel1.fields.qiyeType" placeholder="企业类型" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
      <div class="demo-drawer-footer">
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "qiyeUuid" },
            {
              title: "企业名称",
              key: "qiyeName",
              minWidth: 120,
      ...
      formModel: {
        ...
        rules: {
          ...
          qiyeType: [{ type: "string", required: true, message: "请选择状态" }],
          qiyeAddress: [
            { type: "string", required: true, message: "请输入企业地址" }
          ]
        }
      },
      formModel1: {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.qiyeUuid);
    } //删除
  },
  methods: {
    //页面加载
    loadDispatchList() {
      QiyeList(this.stores.userinfo.query).then(res => {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.qiyeName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.qiyeName)) {
          this.$Message.warning("企业名称不合法!");
          return;
        }
      } else {
      ...
      QiyeCreate(this.formModel.fields).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
    ...
    doEditDispatch() {
      ...
      //this.datadeal();
      QiyeEdit(this.formModel.fields).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.formModel.opened = false; //关闭表单
          this.loadDispatchList();
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/企业信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
    ...
    async handleimport() {
      ...
      this.defaultmsg = "";
      if (this.isexitexcel) {
        await QiyeImport(this.exceldata).then(res => {
          if (res.data.code == 200) {
    ...
    doCuisineExport(command) {
      ...
      QiyeExport(this.selectedRowsId.join(",")).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.formModel.selection = [];
          console.log(res);
  ...
  created() {
    ...
    this.actionurl = config.baseUrl.dev + "api/v1/Qiye/Qiye/UpLoad";
  }
};
</script>
<style>
