<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="政策名" prop="theUnionZhengcName">
            <Input v-model="formModel.fields.theUnionZhengcName" placeholder="政策名" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="政策内容" prop="neirong">
            <Input v-model="formModel.fields.neirong" type="textarea" placeholder="政策内容" />
          </FormItem>
        </Row>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="政策名" prop="theUnionZhengcName">
            <Input v-model="formModel1.fields.theUnionZhengcName" placeholder="政策名" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="政策内容" prop="neirong">
            <Input v-model="formModel1.fields.neirong" placeholder="政策内容" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
      <div class="demo-drawer-footer">
    ...
    >
      <div>
        ...
        <Button
          ...
          title="工会普惠政策信息导入模板下载"
        >工会普惠政策信息导入模板下载</Button>
        <Button
...
import {
  ...
  GetImport, //导入
  GetExport //导出
} from "@/api/TheUnionInfo/TheUnionZhengcInfo";
import config from "@/config";
import { getToken } from "@/libs/util";
export default {
  name: "TheUnionZhengcInfo",
  components: {
    DzTable
  },
  data() {
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "theUnionZhengcUuid" },
            {
              title: "政策名",
              key: "theUnionZhengcName",
              minWidth: 120,
              sortable: true
            },
            {
      ...
      formModel: {
        ...
        rules: {
          theUnionZhengcName: [
            { type: "string", required: true, message: "请输入政策名" }
          ],
        }
      },
      formModel1: {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.theUnionZhengcUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.theUnionZhengcUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.theUnionZhengcUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/工会普惠政策信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
