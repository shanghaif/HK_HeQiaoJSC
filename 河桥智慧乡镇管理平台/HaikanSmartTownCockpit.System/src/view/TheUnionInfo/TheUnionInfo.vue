...
import {
  ...
  batchCommand,
  deletePromoInfo,
} from "@/api/TheUnionInfo/TheUnionInfo";
...
export default {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.theUnionUuid);
    }
  },
  methods: {
    ...
    //单条删除
    handleDelete(row) {
      this.doDelete(row.theUnionUuid);
    },
    doDelete(ids) {
      if (!ids) {
        this.$Message.warning("请选择至少一条数据");
    ...
    handleEdit(row) {
      ...
      this.doLoadPromoInfo(row.theUnionUuid);
    },
    handleShow(row) {
