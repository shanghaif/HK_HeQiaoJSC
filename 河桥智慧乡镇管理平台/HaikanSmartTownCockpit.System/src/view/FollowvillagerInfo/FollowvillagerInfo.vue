<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="姓名" prop="followvillagerName">
            <Input v-model="formModel.fields.followvillagerName" placeholder="姓名" />
          </FormItem>
        </Row>
        <Row :gutter="16">
        ...
        <Row :gutter="16">
          <FormItem label="基本事迹" prop="shiji">
            <Input v-model="formModel.fields.shiji" type="textarea" placeholder="基本事迹" />
          </FormItem>
        </Row>

        <Row :gutter="16">
          <FormItem label="介绍" prop="jieshao">
            <Input v-model="formModel.fields.jieshao" type="textarea" placeholder="介绍" />
          </FormItem>
        </Row>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="姓名" prop="followvillagerName">
            <Input v-model="formModel1.fields.followvillagerName" placeholder="姓名" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="性别" prop="sex">
        ...
        <Row :gutter="16">
          <FormItem label="基本事迹" prop="shiji">
            <Input v-model="formModel1.fields.shiji" type="textarea" placeholder="基本事迹" :readonly="true" />
          </FormItem>
        </Row>

        <Row :gutter="16">
          <FormItem label="介绍" prop="jieshao">
            <Input v-model="formModel1.fields.jieshao" type="textarea" placeholder="介绍" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
      <div class="demo-drawer-footer">
...
import {
  ...
  UserInfoImport, //导入
  UserInfoExport //导出
} from "@/api/FollowvillagerInfo/FollowvillagerInfo";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "followvillagerUuid" },
            {
              title: "姓名",
              key: "followvillagerName",
              minWidth: 80,
              sortable: true
            },
            { title: "性别", key: "sex", minWidth: 80, sortable: true },
            { title: "手机", key: "phone", minWidth: 100, sortable: true },
            { title: "介绍", key: "jieshao", minWidth: 150, sortable: true },
            {
              title: "操作",
              align: "center",
              fixed: "right",
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.followvillagerUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.followvillagerUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.followvillagerUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.followvillagerName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.followvillagerName)) {
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/乡贤信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
