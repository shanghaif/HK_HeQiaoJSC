<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="类型名称" prop="propagandaTypeName">
            <Input v-model="formModel.fields.propagandaTypeName" placeholder="类型名称" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="介绍" prop="title">
            <Input type="textarea"  v-model="formModel.fields.title" placeholder="介绍" />
          </FormItem>
        </Row>
      </Form>
    ...
    <!-- <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="类型名称" prop="propagandaTypeName">
            <Input v-model="formModel1.fields.propagandaTypeName" placeholder="类型名称" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="介绍" prop="title">
            <Input v-model="formModel1.fields.title" placeholder="介绍" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
        ...
        <Row :gutter="16">
          <FormItem label="企业类型" prop="PTypeType">
            <Input v-model="formModel1.fields.PTypeType" placeholder="企业类型" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
      <div class="demo-drawer-footer">
...
import {
  ...
  PTypeImport, //导入
  PTypeExport //导出
} from "@/api/PublicityInfo/PropagandaType";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "propagandaTypeUuid" },
            {
              title: "类型名称",
              key: "propagandaTypeName",
              minWidth: 120,
              sortable: true
            },
            {
      ...
      formModel: {
        ...
        rules: {
          propagandaTypeName: [{ type: "string", required: true, validator: validateName}],
        }
      },
      formModel1: {
        opened: false,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.propagandaTypeUuid);
    } //删除
  },
  methods: {
    //页面加载
    loadDispatchList() {
      PTypeList(this.stores.userinfo.query).then(res => {
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.propagandaTypeUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.propagandaTypeUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.propagandaTypeName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.propagandaTypeName)) {
          this.$Message.warning("类型名称不合法!");
          return;
        }
      } else {
      ...
      PTypeCreate(this.formModel.fields).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
    ...
    doEditDispatch() {
      ...
      //this.datadeal();
      PTypeEdit(this.formModel.fields).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.formModel.opened = false; //关闭表单
          this.loadDispatchList();
    ...
    async handleimport() {
      ...
      this.defaultmsg = "";
      if (this.isexitexcel) {
        await PTypeImport(this.exceldata).then(res => {
          if (res.data.code == 200) {
    ...
    doCuisineExport(command) {
      ...
      PTypeExport(this.selectedRowsId.join(",")).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.formModel.selection = [];
          console.log(res);
  ...
  created() {
    ...
    this.actionurl = config.baseUrl.dev + "api/v1/PType/PType/UpLoad";
  }
};
</script>
<style>
