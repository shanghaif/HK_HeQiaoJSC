<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="类型名称" prop="publicityTypeName">
            <Input v-model="formModel.fields.publicityTypeName" placeholder="类型名称" />
          </FormItem>
        </Row>
        <Row :gutter="16">
    ...
    <!-- <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="类型名称" prop="publicityTypeName">
            <Input v-model="formModel1.fields.publicityTypeName" placeholder="类型名称" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="介绍" prop="title">
...
import {
  ...
  PTypeImport, //导入
  PTypeExport //导出
} from "@/api/PublicityInfo/PTypeInfo";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "publicityTypeUuid" },
            {
              title: "类型名称",
              key: "publicityTypeName",
              minWidth: 120,
              sortable: true
            },
            {
      ...
      formModel: {
        ...
        rules: {
          publicityTypeName: [{ type: "string", required: true, validator: validateName}],
        }
      },
      formModel1: {
        opened: false,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.publicityTypeUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.publicityTypeUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.publicityTypeUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.publicityTypeName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.publicityTypeName)) {
