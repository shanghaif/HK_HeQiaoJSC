<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="妇联组织名称" prop="womenFederationName">
            <Input v-model="formModel.fields.womenFederationName" placeholder="妇联组织名称" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="所在地" prop="address">
            <Input v-model="formModel.fields.address" placeholder="所在地" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="主席" prop="zhuxi">
            <Input v-model="formModel.fields.zhuxi" placeholder="主席" />
          </FormItem>
        </Row>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="妇联组织名称" prop="womenFederationName">
            <Input v-model="formModel1.fields.womenFederationName" placeholder="妇联组织名称" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="所在地" prop="address">
            <Input v-model="formModel1.fields.address" placeholder="所在地" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="主席" prop="zhuxi">
            <Input v-model="formModel1.fields.zhuxi" placeholder="主席" :readonly="true" />
          </FormItem>
        </Row>
...
import {
  ...
  GetImport, //导入
  GetExport //导出
} from "@/api/FuLianInfo/WomenFederation";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "womenFederationUuid" },
            {
              title: "妇联组织名称",
              key: "womenFederationName",
              minWidth: 120,
              sortable: true
            },
            {
      ...
      formModel: {
        ...
        rules: {
          womenFederationName: [
            { type: "string", required: true, message: "请输入妇联组织名称" }
          ],
          address: [
            { type: "string", required: true, message: "请输入所在地" }
          ]
        }
      },
      formModel1: {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.womenFederationUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.womenFederationUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.womenFederationUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.womenFederationName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.womenFederationName)) {
          this.$Message.warning("请勿输入非法字符!");
          return;
        }
      } else {
        this.$Message.warning("妇联组织名称不能为空!");
        return;
      }
    ...
    doEditDispatch() {
      if (this.formModel.fields.womenFederationName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.womenFederationName)) {
          this.$Message.warning("妇联组织名称不合法!");
          return;
        }
      } else {
        this.$Message.warning("妇联组织名称不能为空!");
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/妇联组织信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
