<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="妇女之家名称" prop="womenHouseName">
            <Input v-model="formModel.fields.womenHouseName" placeholder="妇女之家名称" />
          </FormItem>
        </Row>
        <Row :gutter="16">
        ...
        <Row :gutter="16">
          <FormItem label="负责人" prop="zhuxi">
            <Input v-model="formModel.fields.zhuxi" placeholder="负责人" />
          </FormItem>
        </Row>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="妇女之家名称" prop="womenHouseName">
            <Input v-model="formModel1.fields.womenHouseName" placeholder="妇女之家名称" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="所在地" prop="address">
        ...
        <Row :gutter="16">
          <FormItem label="负责人" prop="zhuxi">
            <Input v-model="formModel1.fields.zhuxi" placeholder="负责人" :readonly="true" />
          </FormItem>
        </Row>
...
import {
  ...
  GetImport, //导入
  GetExport //导出
} from "@/api/FuLianInfo/WomenHouse";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "womenHouseUuid" },
            {
              title: "妇女之家名称",
              key: "womenHouseName",
              minWidth: 120,
      ...
      formModel: {
        ...
        rules: {
          womenHouseName: [
            { type: "string", required: true, message: "请输入妇女之家名称" }
          ],
          address: [
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.womenHouseUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.womenHouseUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.womenHouseUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.womenHouseName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.womenHouseName)) {
          this.$Message.warning("请勿输入非法字符!");
          return;
        }
      } else {
        this.$Message.warning("妇女之家名称不能为空!");
        return;
      }
    ...
    doEditDispatch() {
      if (this.formModel.fields.womenHouseName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.womenHouseName)) {
          this.$Message.warning("妇女之家名称不合法!");
          return;
        }
      } else {
        this.$Message.warning("妇女之家名称不能为空!");
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/妇女之家信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
