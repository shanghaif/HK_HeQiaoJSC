<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="店名" prop="canyinName">
            <Input v-model="formModel.fields.canyinName" placeholder="店名" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="所处位置" prop="canyinAddress">
            <Input v-model="formModel.fields.canyinAddress" placeholder="所处位置" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="店主" prop="fuzeren">
            <Input v-model="formModel.fields.fuzeren" placeholder="店主" />
          </FormItem>
        </Row>
        <Row :gutter="16">
        ...
        <Row :gutter="16">
          <FormItem label="状态" prop="staues">
            <Select
              filterable
              v-model="formModel.fields.staues"
              style="width:180px"
              placeholder="状态"
            >
              <Option
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="店名" prop="canyinName">
            <Input v-model="formModel1.fields.canyinName" placeholder="店名" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="所处位置" prop="canyinAddress">
            <Input v-model="formModel1.fields.canyinAddress" placeholder="所处位置" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="店主" prop="fuzeren">
            <Input v-model="formModel1.fields.fuzeren" placeholder="店主" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="联系电话" prop="phone">
            <Input v-model="formModel1.fields.phone" placeholder="联系电话" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="状态" prop="staues">
            <Input v-model="formModel1.fields.staues" placeholder="状态" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
      <div class="demo-drawer-footer">
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "canyinUuid" },
            {
              title: "店名",
              key: "canyinName",
              minWidth: 120,
      ...
      formModel: {
        ...
        rules: {
          canyinName: [
            { type: "string", required: true, message: "请输入店名" }
          ],
          canyinAddress: [
            { type: "string", required: true, message: "请输入所处位置" }
          ],
          staues: [{ type: "string", required: true, message: "请选择状态" }]
        }
      },
      formModel1: {
        opened: false,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.canyinUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.canyinUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.canyinName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.canyinName)) {
          this.$Message.warning("请勿输入非法字符!");
          return;
        }
      } else {
        this.$Message.warning("店名不能为空!");
        return;
      }
      if (this.formModel.fields.canyinAddress != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.canyinAddress)) {
          this.$Message.warning("请勿输入非法字符!");
          return;
        }
      } else {
        this.$Message.warning("所处位置不能为空!");
        return;
      }
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/餐饮信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
