      ...
      >
        <Row :gutter="16">
          <FormItem label="姓名" prop="organName">
            <Input
              v-model="formModel.fields.organName"
              :readonly="checkShow()"
              placeholder="请输入姓名"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="监护人" prop="tutelageName">
            <Input v-model="formModel.fields.tutelageName" :readonly="checkShow()" placeholder="请输入监护人" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="监护人联系方式" prop="tutelagePhone">
            <Input v-model="formModel.fields.tutelagePhone" :readonly="checkShow()" placeholder="请输入监护人联系方式" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="家庭地址" prop="houseAddress">
            <Input v-model="formModel.fields.houseAddress" :readonly="checkShow()" placeholder="请输入家庭地址" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="具体情况" prop="specific">
            <Input type="textarea" v-model="formModel.fields.specific" :readonly="checkShow()" placeholder="请输入具体情况" />
          </FormItem>
        </Row>
      </Form>
...
export default {
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "organPeoInfoUuid" },
            { title: "姓名", key: "organName" },
            { title: "性别", key: "sex" },
            { title: "出生日期", key: "birth" },
            { title: "监护人", key: "tutelageName" },
            { title: "监护人联系方式", key: "tutelagePhone" },
            { title: "家庭地址", key: "houseAddress", ellipsis: true, tooltip: true },
            {
              title: "操作",
              align: "center",
              width: 200,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.organPeoInfoUuid);
    }, //删除
  },
  methods: {
    //页面加载
    ...
    handleDetail(e) {
      ...
      this.doLoadData(e.organPeoInfoUuid);
    },
    //添加按钮
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.organPeoInfoUuid);
    },
    //查询当前行信息
    doLoadData(id) {
      GetShow(id).then((res) => {
    ...
    handleSubmitConsumable() {
      if(this.formModel.fields.organName!=""){
        let reg = /^([\u4e00-\u9fa5]){2,7}$/;
      ...
      if(this.formModel.fields.tutelagePhone!=""){
        let reg2 = /^[1][3,4,5,7,8][0-9]{9}$/;
        if (!reg2.test(this.formModel.fields.tutelagePhone)) {
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.organPeoInfoUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleimportmodel() {
      window.location.href =
        this.url + "UploadFiles/ImportBuidingModel/留守儿童信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
  ...
  created() {
    ...
    this.actionurl = config.baseUrl.dev + "api/v1/Organization/Oldman/UpLoad";
  },
};
</script>
<style>
