<template>
  <div>
      ...
      >
        ...
        >
          <template
            ...
            slot="action"
            v-if="row.organizationName!='妇联'&& row.organizationName!='总工会' && row.organizationName!='共青团'"
          >
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="工会名称" prop="organizationName">
            <Input
              v-model="formModel.fields.organizationName"
              :disabled="checkShow()"
              placeholder="请输入工会名称"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="工会简介" prop="organizationRemarks">
            <Input
              v-model="formModel.fields.organizationRemarks"
              :disabled="checkShow()"
              placeholder="请输入工会名称"
              type="textarea"
            />
    ...
    <Drawer title="人员信息" v-model="stores1.opened" width="80%" :mask-closable="false" :mask="true" @on-close="onclose">
      <Card>
        <dz-table
        ...
        >
              ...
              <Row :gutter="16">
                  ...
                  <Form inline @submit.native.prevent>
                    <FormItem>
                      <Input
                        ...
                        search
                        :clearable="true"
                        v-model="stores1.demo.query.kw"
                        placeholder="请输入姓名"
                        @on-search="ohandleSearchDispatch()"
                      ></Input>
                    </FormItem>
                  </Form>
                </Col>
                <Col span="14" class="dnc-toolbar-btns">
                  <ButtonGroup class="mr3">
                    ...
                    ></Button>
                    <Button icon="md-refresh" title="刷新" @click="ohandleRefresh"></Button>
                  </ButtonGroup>
                  <Button
                    v-can="'create1'"
                    icon="md-create"
                  ...
                  <Button
                    ...
                    icon="ios-cloud-download-outline"
                    type="primary"
                    @click="ohandleExportCuisine('export')"
                    title="导出"
                  >导出</Button>
                  <Button
                    v-can="'yjexport1'"
                    icon="ios-cloud-download-outline"
                    type="primary"
                    @click="ohandleExportCuisineyj('export')"
                    title="一键导出"
                  >一键导出</Button>
                </Col>
              </Row>
          ...
          >
            <template slot-scope="{ row, index }" slot="action">
              <Poptip confirm :transfer="true" title="确定要删除吗?" @on-ok="ohandleDelete(row)">
                <Tooltip placement="top" content="删除" :delay="1000" :transfer="true">
                  <Button
                    type="error"
                    v-can="'deletes1'"
    ...
    <Drawer
      ...
      >
        ...
        >
          <Row :gutter="16">
            <FormItem label="姓名" prop="organName">
              <Input
                v-model="formModel1.fields.organName"
                :readonly="ocheckShow()"
                placeholder="请输入姓名"
              />
            </FormItem>
          ...
          <Row :gutter="16">
            <FormItem label="身份证号码" prop="identityCard">
              <Input
                v-model="formModel1.fields.identityCard"
                :readonly="ocheckShow()"
          ...
          <Row :gutter="16">
            <FormItem label="工作" prop="zjWork">
              <Input
                v-model="formModel1.fields.zjWork"
                :readonly="ocheckShow()"
                placeholder="请输入工作"
              />
            </FormItem>
          ...
          <Row :gutter="16">
            <FormItem label="职位" prop="duty">
              <Input v-model="formModel1.fields.duty" :readonly="ocheckShow()" placeholder="请输入职位" />
            </FormItem>
          </Row>
          <Row :gutter="16">
            <FormItem label="地址" prop="houseAddress">
              <Input
                v-model="formModel1.fields.houseAddress"
                type="textarea"
                :readonly="ocheckShow()"
                placeholder="请输入地址"
              />
...
import {
  ...
  oGetShow, //获取选定信息
  obatchCommand, //批量删除
  odeleteDepartment, //单个删除
  oGetEdit, //编辑
  oGetImport, //导入
  oGetExportPass, //导出
} from "@/api/organization/organinfo";
...
export default {
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "organizationUuid" },
            { title: "工会名称", key: "organizationName" },
            { title: "工会人数", key: "organizationPeople" },
            {
              title: "操作",
              align: "center",
              width: 200,
      ...
      formModel: {
        ...
        rules: {
          organizationName: [
            { type: "string", required: true, message: "工会名称不能为空" },
          ],
        },
      },
      stores1: {
        ...
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "organPeoInfoUuid",_disabled:true},
            { title: "姓名", key: "organName" },
            { title: "性别", key: "sex" },
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.organizationUuid);
    }, //删除
    oformTitle() {
    ...
    oselectedRowsId() {
      return this.formModel1.selection.map((x) => x.organPeoInfoUuid);
    }, //删除
  },
  methods: {
    //页面加载
    ...
    //详情显示
    handleDetail(e) {
      this.stores1.demo.query.kw1 = e.organizationUuid;
      this.formModel1.fields.organizationUuid = e.organizationUuid;
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.organizationUuid);
    },
    //查询当前行信息
    doLoadData(id) {
      GetShow(id).then((res) => {
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.organizationUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleBatchCommand(command) {
      ...
      let data=this.formModel.selection.map((x) => x.organizationName);
      // for(let k=0;k<data.length;k++){
      //     if(data[k]=="妇联"){
      //         this.$Message.warning("请勿选择妇联");
      //         return;
      //     }
      //     if(data[k]=="总工会"){
      //         this.$Message.warning("请勿选择总工会");
      //         return;
      //     }
      //     if(data[k]=="共青团"){
      //         this.$Message.warning("请勿选择共青团");
      //         return;
      //     }
      // }
      this.$Modal.confirm({
    ...
    handleimportmodel() {
      window.location.href =
        this.url + "UploadFiles/ImportBuidingModel/工会导入模板.xlsx";
    },
    //导入
    importfxx(e) {
    ...
    //页面加载
    oloadDispatchList() {
      oGetList(this.stores1.demo.query).then((res) => {
    ...
    ohandleDetail(e) {
      ...
      this.odoLoadData(e.organPeoInfoUuid);
    },
    //添加按钮
    ...
    ohandleEdit(row) {
      ...
      this.formModel1.opened = true;
      this.ohandleResetFormDispatch(); //清空表单
      this.odoLoadData(row.organPeoInfoUuid);
    },
    //查询当前行信息
    odoLoadData(id) {
      oGetShow(id).then((res) => {
        if (res.data.code === 200) {
    ...
    ohandleSubmitConsumable() {
      if(this.formModel1.fields.organName!=""){
        let reg = /^([\u4e00-\u9fa5]){2,7}$/;
        if (!reg.test(this.formModel1.fields.organName)) {
      ...
      if(this.formModel1.fields.identityCard!=""){
        let reg1 = /^(^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$)|(^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[Xx])$)$/;
        if (!reg1.test(this.formModel1.fields.identityCard)) {
          this.$Message.warning("身份证号不合法!");
          return;
        }
      }
      if(this.formModel1.fields.phone!=""){
        let reg2 = /^[1][3,4,5,7,8][0-9]{9}$/;
        if (!reg2.test(this.formModel1.fields.phone)) {
    ...
    //添加（保存）
    odocreateDispatch() {
      oGetCreate(this.formModel1.fields).then((res) => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
    ...
    //编辑（保存）
    odoEditDispatch() {
      this.formModel1.fields.birth = new Date(
        Date.parse(new Date(this.formModel1.fields.birth)) + 8 * 3600 * 1000
      );
      oGetEdit(this.formModel1.fields).then((res) => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.formModel1.opened = false; //关闭表单
          this.stores1.opened = true;
    ...
    //导入相关操作
    ohandleImportCuisine() {
      this.osuccessmsg = "";
      this.orepeatmsg = "";
      this.odefaultmsg = "";
      this.$refs.oinputer.value = "";
      this.oisexitexcel = false;
      this.oformimport.opened = true;
    },
    //下载模板
    ohandleimportmodel() {
      window.location.href =
        this.url + "UploadFiles/ImportBuidingModel/工会人员信息导入模板.xlsx";
    },
    //导入
    oimportfxx(e) {
      let inputDOM = this.$refs.oinputer;
      //声明一个FormDate对象
      let formData = new FormData();
      let file = inputDOM.files[0];
      let AllUpExt = ".xls|.xlsx|";
      ...
      if (AllUpExt.indexOf(extName + "|") == "-1") {
        ...
        if (file != null && file != undefined) {
          ...
          formData.append("excelFile", file);
          this.exceldata = formData;
          this.exceldata.append("guid", this.formModel1.fields.organizationUuid);
        } else {
          this.isexitexcel = false;
        }
      }
    ...
    async ohandleimport() {
      this.oimportdisable = true;
      this.osuccessmsg = "";
      this.orepeatmsg = "";
      this.odefaultmsg = "";
      if (this.isexitexcel) {
        await oGetImport(this.exceldata).then((res) => {
          if (res.data.code == 200) {
            this.otime = res.data.data.time;
            this.osuccessmsg = res.data.data.successmsg;
            this.orepeatmsg = res.data.data.repeatmsg;
            this.odefaultmsg = res.data.data.defaultmsg;
            this.$Message.success(res.data.message);
            this.oloadDispatchList();
            this.oformimport.opened = false;
            this.stores1.opened = true;
          } else {
            this.$Message.warning(res.data.message);
          }
      ...
      this.oimportdisable = false;
    },
    //导出
    ohandleExportCuisine(command) {
      if (!this.oselectedRowsId || this.oselectedRowsId.length <= 0) {
        this.$Message.warning("请选择至少一条数据");
        return;
      }
      this.$Modal.confirm({
        ...
        loading: true,
        onOk: () => {
          this.odoExport(command);
        },
      });
    },
    odoExport(command) {
        let data={ids:this.oselectedRowsId.join(","),guid:this.formModel1.fields.organizationUuid};
      oGetExportPass(data).then((res) => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.formModel1.selection = [];
          this.stores1.opened = true;
    ...
    //一键导出
    ohandleExportCuisineyj(command) {
      this.$Modal.confirm({
        title: "操作提示",
        content:
          "<p>确定要执行当前 [" +
  ...
  created() {
    ...
    this.actionurl =
      config.baseUrl.dev + "api/v1/Organization/OrganInfo/UpLoad";
    this.opostheaders = {
      Authorization: "Bearer " + getToken(),
      //Accept: "application/json, text/plain, */*"
    };
    this.oactionurl =
      config.baseUrl.dev + "api/v1/Organization/OrganInfo/oUpLoad";
  },
};
</script>
<style>
