<template>
  <div>
    <Card>
      <dz-table
        :totalCount="stores.xuncha.query.totalCount"
        :pageSize="stores.xuncha.query.pageSize"
        :currentPage="stores.xuncha.query.currentPage"
      ...
      >
            ...
            <Row :gutter="16">
                ...
                <Form inline @submit.native.prevent>
                  <FormItem>
                    <Input
                      ...
                      search
                      :clearable="true"
                      v-model="stores.xuncha.query.kw"
                      placeholder="输入巡查人/巡查地点搜索..."
                      @on-search="handleSearchXuncha()"
                    >
                      <Select
                        slot="prepend"
                        v-model="stores.xuncha.query.isDeleted"
                        @on-change="handleSearchXuncha"
                        placeholder="删除状态"
                        style="width:60px;"
                      >
                        <Option
                          v-for="item in stores.xuncha.sources.isDeletedSources"
                          :value="item.value"
                          :key="item.value"
                        >{{item.text}}</Option>
                      </Select>
              ...
              <Col span="8" class="dnc-toolbar-btns">
                ...
                <Button
                  ...
                  type="primary"
                  @click="handleExportXuncha('export')"
                  title="导出"
                >导出</Button>
                <Button
                  v-can="'yjexport'"
                  icon="ios-cloud-download-outline"
                  type="primary"
                  @click="handleExportXunchayj('export')"
                  title="一键导出"
                >一键导出</Button>
              </Col>
            </Row>
        ...
        <Table
          ...
          size="small"
          :highlight-row="true"
          :data="stores.xuncha.data"
          :columns="stores.xuncha.columns"
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="情况上报" prop="qingkShangbao">
              <Input
                v-model.trim="formModel.fields.qingkShangbao"
                placeholder="请输入情况"
                style="width: 400px"
                :maxlength="200"
              />
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="状态" prop="staues">
              <Input
                v-model.trim="formModel.fields.staues"
                placeholder="请输入状态"
                style="width: 400px"
                :maxlength="200"
              />
      ...
      <div style="margin-top: 100px">
        <Button icon="md-checkxuncha-circle" type="primary" @click="handleSubmitPlan">保 存</Button>
...
<script>
  ...
  import {
    ...
    XunchaImport,
    XunchaExport
  } from "@/api/emergency/emergencyresponse/xuncha";
  ...
  export default {
      ...
      return {
        ...
        formModel: {
          ...
          rules: {
            ...
            qingkShangbao: [{type: "string",required: true,message: "请输入情况",trigger:'blur'}],
            staues: [{type: "string",required: true,message: "请输入状态",trigger:'blur'}],
        ...
        stores: {
          xuncha: {
            ...
            columns: [
              ...
              { title: "情况上报", key: "qingkShangbao" , sortable: true},
              { title: "联系电话", key: "fuzerPhone" , sortable: true},
              { title: "状态", key: "staues" , sortable: true},
              {
                title: "操作",
                fixed: "right",
                align: "center",
    ...
    computed: {
      ...
      selectedRowsId() {
        return this.formModel.selection.map(x => x.xunchaInfoUuid);
      }
    },
    methods: {
      loadXunchaList() {
        getXunchaList(this.stores.xuncha.query).then(res => {
          this.stores.xuncha.data = res.data.data;
          this.stores.xuncha.query.totalCount = res.data.totalCount;
          //console.log(this.stores.educaplan.data);
        });
      },
      handleSearchXuncha() {
        this.stores.xuncha.query.currentPage = 1;
        this.loadXunchaList();
      },
      handleRefresh() {
        this.stores.xuncha.query.currentPage = 1;
      ...
      docreateXuncha() {
        createXuncha(this.formModel.fields).then(res => {
      ...
      doEditPlan() {
        editXuncha(this.formModel.fields).then(res => {
          if (res.data.code === 200) {
            this.$Message.success(res.data.message);
            this.handleCloseFormWindow();
            this.loadXunchaList();
      ...
      //单条删除
      handleDelete(row) {
        this.doDelete(row.xunchaInfoUuid);
      },
      doDelete(ids) {
        if (!ids) {
          this.$Message.warning("请选择至少一条数据");
      ...
      handleEdit(row) {
        ...
        this.doLoadXuncha(row.xunchaInfoUuid);
      },
      ...
      doLoadXuncha(guid) {
        loadXuncha({ guid: guid }).then(res => {
          this.formModel.fields = res.data.data;
        });
      },
      handlePageChanged(page) {
        this.stores.xuncha.query.currentPage = page;
        this.loadXunchaList();
      },
      handlePageSizeChanged(pageSize) {
        this.stores.xuncha.query.pageSize = pageSize;
        this.loadXunchaList();
      },
      ...
      handleimportmodel() {
        ...
        window.location.href =
          this.url + "UploadFiles/ImportEmergencyModel/巡查记录信息导入模板.xlsx";
      },
      //导入
      importfxx(e) {
      ...
      async handleImportXuncha() {
        this.importdisable = true;
        this.successmsg = "";
        this.repeatmsg = "";
        this.defaultmsg = "";
        if (this.isexitexcel) {
          await XunchaImport(this.exceldata).then(res => {
            if (res.data.code == 200) {
      ...
      handleExportXuncha(command) {
        ...
        this.$Modal.confirm({
          ...
          loading: true,
          onOk: () => {
            this.doXunchaExport(command);
          }
        });
      },
      //一键导出
      handleExportXunchayj(command) {
        this.$Modal.confirm({
          title: "操作提示",
          content:
            "<p>确定要执行当前 [" +
      ...
      doXunchaExport(command) {
        XunchaExport(this.selectedRowsId.join(",")).then(res => {
          if (res.data.code === 200) {
            this.$Message.success(res.data.message);
            this.formModel.selection = [];
