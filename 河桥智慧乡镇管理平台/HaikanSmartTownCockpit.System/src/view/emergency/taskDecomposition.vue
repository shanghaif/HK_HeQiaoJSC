<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="总指挥" prop="commander">
            <Input
              v-model="formModel.fields.commander"
              :disabled="checkShow()"
              placeholder="请输入总指挥姓名"
            />
          </FormItem>
        </Row>
        <!-- <Row :gutter="16">
        ...
        <Row :gutter="16">
          <FormItem label="成员" prop="member">
            <Input
              v-model="formModel.fields.member"
              :disabled="checkShow()"
              placeholder="请输入成员姓名"
            />
          </FormItem>
    ...
    <Drawer
      ...
      >
        ...
        >
          <Row :gutter="16">
            <FormItem label="工作专班" prop="specialWorkTeam">
              <Input
                v-model="formModel1.fields.specialWorkTeam"
                :readonly="ocheckShow()"
                placeholder="请输入工作专班"
              />
            </FormItem>
          ...
          <Row :gutter="16">
            <FormItem label="专组长班" prop="dedicatedTeamLeader">
              <Input
                v-model="formModel1.fields.dedicatedTeamLeader"
                :readonly="ocheckShow()"
                placeholder="请输入专组长班"
              />
            </FormItem>
          ...
          <Row :gutter="16">
            <FormItem label="相关干部" prop="relatedCadres">
              <Input
                v-model="formModel1.fields.relatedCadres"
                :readonly="ocheckShow()"
                placeholder="请输入相关干部"
              />
            </FormItem>
          ...
          <Row :gutter="16">
            <FormItem label="村,企业负责人员" prop="principal">
              <Input
                v-model="formModel1.fields.principal"
                :readonly="ocheckShow()"
                placeholder="请输入村企业负责人员"
              />
            </FormItem>
          </Row>
          <Row :gutter="16">
            <FormItem label="工作要求" prop="requirement">
              <Input
                type="textarea" :autosize="true"
                v-model="formModel1.fields.requirement"
...
} from "@/api/emergency/decomposition";
...
export default {
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "taskDecompositionUuid" },
            { title: "总指挥", key: "commander" },
            { title: "成员", key: "member" },
            {
              title: "操作",
              align: "center",
          ...
          columns: [
            {
              type: "selection",
              width: 50,
              key: "taskDecompInfoUuid",
              _disabled: true
            },
            { title: "工作专班", key: "specialWorkTeam" },
            { title: "专组长班", key: "dedicatedTeamLeader" },
            { title: "相关干部", key: "relatedCadres" },
            { title: "村,企业负责人员", key: "principal" },
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.taskDecompositionUuid);
    }, //删除
    oformTitle() {
    ...
    oselectedRowsId() {
      return this.formModel1.selection.map(x => x.taskDecompInfoUuid);
    } //删除
  },
  methods: {
    //页面加载
    loadDispatchList() {
      getDecompositionList(this.stores.demo.query).then(res => {
    ...
    handleDetail(e) {
      console.log(e);
      this.stores1.demo.query.tduuid = e.taskDecompositionUuid;
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.taskDecompositionUuid);
    },
    //查询当前行信息
    doLoadData(id) {
      loadDecomposition(id).then(res => {
    ...
    ohandleDetail(e) {
      ...
      this.odoLoadData(e.taskDecompInfoUuid);
    },
    //添加按钮
    ...
    ohandleEdit(row) {
      ...
      this.formModel1.opened = true;
      this.ohandleResetFormDispatch(); //清空表单
      this.odoLoadData(row.taskDecompInfoUuid);
    },
    //查询当前行信息
    odoLoadData(id) {
    ...
    //添加（保存）
    odocreateDispatch() {
      this.formModel1.fields.tduuid=this.stores1.demo.query.tduuid;
