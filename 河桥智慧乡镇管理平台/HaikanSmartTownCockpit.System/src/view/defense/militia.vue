      ...
      >
        <Row :gutter="16">
          <FormItem label="姓名" prop="realName">
            <Input v-model="formModel.fields.realName" :disabled="checkShow()" placeholder="请输入姓名" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="职务" prop="residence">
            <Input
        ...
        <Row :gutter="16">
          <FormItem label="联系方式" prop="phone">
            <Input v-model="formModel.fields.phone" :disabled="checkShow()" placeholder="请输入联系方式" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="政治面貌" prop="nation">
            <Input v-model="formModel.fields.nation" :disabled="checkShow()" placeholder="请输入政治面貌" />
          </FormItem>
        </Row>
        <Row :gutter="16">
        ...
        <Row :gutter="16">
          <FormItem label="户籍地" prop="domicile">
            <Input v-model="formModel.fields.domicile" :disabled="checkShow()" placeholder="请输入户籍地" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="工作单位或家庭地址" prop="address">
            <Input
              v-model="formModel.fields.address"
              :disabled="checkShow()"
              placeholder="请输入工作单位或家庭地址"
            />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="班级" prop="wechat">
            <Input v-model="formModel.fields.wechat" :disabled="checkShow()" placeholder="请输入班级" />
          </FormItem>
        </Row>
      </Form>
...
export default {
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "cunzhenmbUuid" },
            { title: "姓名", key: "realName" },
            { title: "联系方式", key: "phone" },
            { title: "职务", key: "residence" },
            {
              title: "操作",
              align: "center",
              width: 200,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.cunzhenmbUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    handleDetail(e) {
      ...
      this.doLoadData(e.cunzhenmbUuid);
    },
    //添加按钮
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.cunzhenmbUuid);
    },
    //查询当前行信息
    doLoadData(id) {
      GetShow(id).then(res => {
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.cunzhenmbUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleimportmodel() {
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/村镇民兵信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
  ...
  created() {
    ...
    this.actionurl = config.baseUrl.dev + "api/v1/Defense/Militia/UpLoad";
  }
};
</script>
<style>
