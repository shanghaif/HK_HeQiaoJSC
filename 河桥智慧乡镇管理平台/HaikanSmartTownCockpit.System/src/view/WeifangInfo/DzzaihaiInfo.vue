<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="地区" prop="diqu">
            <Input v-model="formModel.fields.diqu" placeholder="地区" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="数量" prop="shuliang">
            <Input v-model="formModel.fields.shuliang" placeholder="数量" />
          </FormItem>
        </Row>
      </Form>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="地区" prop="diqu">
            <Input v-model="formModel1.fields.diqu" placeholder="地区" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="数量" prop="shuliang">
            <Input v-model="formModel1.fields.shuliang" placeholder="数量" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
      <div class="demo-drawer-footer">
...
import {
  ...
  GetImport, //导入
  GetExport //导出
} from "@/api/WeifangInfo/DzzaihaiInfo";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "dzhizaihaiInfoUuid" },
            {
              title: "地区",
              key: "diqu",
              minWidth: 120,
      ...
      formModel: {
        ...
        rules: {
          diqu: [{ type: "string", required: true, message: "地区不能为空" }]
        }
      },
      formModel1: {
        opened: false,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.dzhizaihaiInfoUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.dzhizaihaiInfoUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.dzhizaihaiInfoUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.diqu != "") {
        let reg = /^([\u4e00-\u9fa5]){2,7}$/;
        if (!reg.test(this.formModel.fields.diqu)) {
          this.$Message.warning("地区不合法!");
          return;
        }
      } else {
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url +
        "UploadFiles/ImportUserInfoModel/地质灾害点信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
