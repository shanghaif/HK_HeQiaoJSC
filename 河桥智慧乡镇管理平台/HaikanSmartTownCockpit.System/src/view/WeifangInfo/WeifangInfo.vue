<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="户主" prop="dangerousMaster">
            <Input v-model="formModel.fields.dangerousMaster" placeholder="户主" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="联系电话" prop="dangerousPhone">
            <Input v-model="formModel.fields.dangerousPhone" placeholder="联系电话" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="位置" prop="dangerousAddress">
            <Input v-model="formModel.fields.dangerousAddress" placeholder="位置" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="网格员姓名" prop="administratorName">
            <Input v-model="formModel.fields.administratorName" placeholder="网格员姓名" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="网格员电话" prop="administratorPhone">
            <Input v-model="formModel.fields.administratorPhone" placeholder="网格员电话" />
          </FormItem>
        </Row>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="户主" prop="dangerousMaster">
            <Input v-model="formModel1.fields.dangerousMaster" placeholder="户主" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="联系电话" prop="dangerousPhone">
            <Input v-model="formModel1.fields.dangerousPhone" placeholder="联系电话" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="位置" prop="dangerousAddress">
            <Input v-model="formModel1.fields.dangerousAddress" placeholder="位置" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="网格员姓名" prop="administratorName">
            <Input
              v-model="formModel1.fields.administratorName"
              placeholder="网格员姓名"
              :readonly="true"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="网格员电话" prop="administratorPhone">
            <Input
              v-model="formModel1.fields.administratorPhone"
              placeholder="网格员电话"
              :readonly="true"
            />
          </FormItem>
...
import {
  ...
  GetImport, //导入
  GetExport //导出
} from "@/api/WeifangInfo/WeifangInfo";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "dangerousRoomUuid" },
            {
              title: "户主",
              key: "dangerousMaster",
              minWidth: 120,
      ...
      formModel: {
        ...
        rules: {
          dangerousMaster: [
            { type: "string", required: true, message: "户主不能为空" }
          ],
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.dangerousRoomUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.dangerousRoomUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.dangerousRoomUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.dangerousMaster != "") {
        let reg = /^([\u4e00-\u9fa5]){2,7}$/;
        if (!reg.test(this.formModel.fields.dangerousMaster)) {
          this.$Message.warning("户主不合法!");
          return;
        }
      } else {
        this.$Message.warning("户主不能为空!");
        return;
      }
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/危房信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
