<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="行政村" prop="education">
            <Input v-model="formModel.fields.education" :readonly="checkShow()" placeholder="行政村" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="单位名称" prop="studentName">
            <Input
              v-model="formModel.fields.studentName"
              :readonly="checkShow()"
              placeholder="单位名称"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="负责人" prop="project">
            <Input v-model="formModel.fields.project" :readonly="checkShow()" placeholder="负责人" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="联系电话" prop="policy">
            <Input v-model="formModel.fields.policy" :readonly="checkShow()" placeholder="联系电话" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="具体地址" prop="address">
            <Input v-model="formModel.fields.address" :readonly="checkShow()" placeholder="具体地址" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="备注" prop="condition">
            <Input
              type="textarea"
              v-model="formModel.fields.condition"
              :readonly="checkShow()"
              placeholder="备注"
            />
          </FormItem>
...
export default {
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "studentUuid" },
            { title: "行政村", key: "education" },
            { title: "单位名称", key: "studentName" },
            { title: "负责人", key: "project" },
            { title: "联系电话", key: "policy" },
            {
              title: "操作",
              align: "center",
              width: 200,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.studentUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.studentUuid);
    },
    //查询当前行信息
    doLoadData(id) {
      GetShow(id).then(res => {
    ...
    handleSubmitConsumable() {
      if (this.formModel.fields.studentName != "") {
        let reg = /^([\u4e00-\u9fa5]){2,7}$/;
        if (!reg.test(this.formModel.fields.studentName)) {
    ...
    doEditDispatch() {
      ...
      if (
        this.formModel.fields.headTime != "" &&
        this.formModel.fields.headTime != null
      ) {
        this.formModel.fields.headTime = new Date(
          Date.parse(new Date(this.formModel.fields.headTime)) + 8 * 3600 * 1000
        );
      }
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/返乡创业信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
  ...
  created() {
    ...
    this.actionurl =
      config.baseUrl.dev + "api/v1/Runbusiness/Runbusiness/UpLoad";
  }
};
</script>
<style>
