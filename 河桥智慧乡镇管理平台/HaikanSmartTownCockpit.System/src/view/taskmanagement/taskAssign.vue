  ...
  <div>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <Col span="50">
              <FormItem label="指定人员" prop="participant2">
             <Select
                v-model="formModel2.fields.participant2"
                placeholder="指定人员"
                filterable
                multiple
              >
                <Option v-for="item in participantlist" :value="item.value">{{item.label}}</Option>
              </Select>
            </FormItem>
          </Col>
        </Row>
      ...
      <div class="demo-drawer-footer">
        <Button icon="md-checkmark-circle" type="primary" @click="zhiding">保存</Button>
        <Button icon="md-close" @click="formModel2.opened = false">取消</Button>
      </div>
    </Drawer>
...
export default {
      ...
      stores: {
        ...
        columns: [
          ...
          //{ title: "结束时间", key: "finishTime" },
          {
            ...
            render:(h,params)=>{
              debugger
              var dename = params.row.administrativeOffice;
              var mydename = this.$store.state.user.departmentname
              if (dename.indexOf(mydename)!=-1) {
                return h('div', [
                                h('Button', {
                                    props: {
                                        type: 'primary',
  ...
  methods: {
    ...
    handleShowCreateWindow() {
      ...
      this.formModel.fields.missiontype="0";//任务类型为上级交办
    ...
    lookEwq(row) {
      ...
      loaddata({ id: row.id }).then((res) => {
        ...
        this.formModel2.fields.participant2=res.data.data.participant.split(',');
      ...
      zhidingrenlist({id:this.$store.state.user.department}).then((res)=>{
    ...
    zhiding(){
      zhidingren(this.formModel2.fields).then((res)=>{
        if (res.data.code===200) {
          this.$Message.success(res.data.message);
