  ...
  <div>
      ...
      >
        ...
        >
          ...
          <template slot-scope="{ row, index }" slot="action">
			...
			<Tooltip placement="top" v-if="shenpishow(row)" content="审批" :delay="1000" :transfer="true">
			  <Button
			  v-can="'approve'"
			    type="primary"
			    size="small"
      ...
      <Form :model="formModel.fields" ref="formtask" :rules="formModel.rules" label-position="top">
        <Row :gutter="16">
          ...
          <FormItem label="任务类型" prop="missiontype">
            <Select v-model="formModel.fields.missiontype">
                <Option value="0">上级交办</Option>
                <Option value="1">下级上传</Option>
              </Select>
          </FormItem>
          </Col>
        </Row>
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="科室/村庄" prop="administrativeOffice">
              ...
              <Select
                v-model="formModel.fields.administrativeOffice"
                placeholder="科室/村庄"
                filterable
              >
    ...
    >
      ...
      >
        ...
        <Row :gutter="16" v-if="qingkuang">
          <Col span="50">
            <FormItem label="完成情况" prop="remark">
              <Input
        ...
        <Row :gutter="16" v-if="qingkuang">
          <FormItem label="附件">
            <Button
              ...
              size="small"
              type="primary"
              @click="handleimportmodel(formModel2.fields.fujian)"
              title="下载"
              下载
            ></Button>
            <Input v-model="formModel2.fields.fujian" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16" v-if="yijianshow">
          <Col span="50">
            <FormItem label="审核意见" prop="auditOpinions">
              <Input
	  ...
	  <div class="demo-drawer-footer" v-if="shenhea">
	    <Button icon="md-checkmark-circle" type="primary" style="margin-right: 10px;" @click="handok">通 过</Button>

      <Button type="warning" icon="md-close" style="margin-right: 10px;" @click="shenpidaiban">代 办</Button>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="任务标题" prop="missionHeadline">
              <Input
                ...
                :title="formModel3.fields.missionHeadline"
                :readonly="true"
                placeholder="未知..."
              />
            </FormItem>
          ...
          <Col span="12">
            <FormItem label="等级" prop="priority">
              <Input v-model="formModel3.fields.priority" :readonly="true" placeholder="未知..." />
            </FormItem>
          </Col>
        </Row>
        <Row :gutter="16">
          ...
          <Col span="12">
            <FormItem label="科室/村庄" prop="keshiname">
              <Input v-model="formModel3.fields.keshiname" :readonly="true" placeholder="未知..." />
            </FormItem>
          </Col>
        </Row>
        <Row :gutter="16">
        ...
        <Row :gutter="16">
          <Col span="50">
            <FormItem label="完成情况" prop="remark">
              <Input
                v-model="formModel3.fields.remark"
                placeholder="完成情况"
                type="textarea"
                :rows="5"
              />
...
import { getdatalist } from "@/api/taskpai/wofuzetask";

import { keshidata, RegistPicture,keshidata2 } from "@/api/taskpai/taskapis2";
    ...
    return {
      ...
      formModel: {
        ...
        rules: {
          ...
          missiontype: [
            { type: "string", required: true, message: "请选择任务类型" },
          ],
          missionDescribe: [
      ...
      formModel2: {
        ...
        rules: {
          auditOpinion: [
            { type: "string", required: true, message: "请输入审核意见" },
          ],
        },
      },
      formModel3: {
        ...
        rules: {
          remark: [
            { type: "string", required: true, message: "请输入完成情况" },
          ],
        },
      },
      formModelcanyu: {
...
getauditStatus(row) {
    ...
    shenpishow(row){
      var show=false;
      if(row.shenpishow=='是'){//状态为审核中显示审批
        show=true;
      }
      return show;
    },
        ...
        this.qingkuang=false;
		 ...
		 loaddata({ id: row.id}).then(res=>{
			 this.formModel2.fields=res.data.data
				...
				validateForm2() {
	  			  let _valid = false;
	  			  this.$refs["formtasklook"].validate(valid => {
	  			    if (!valid) {
	  			      this.$Message.error("请完善表单信息");
	  			    } else {
	  			      _valid = true;
          ...
          validateForm3() {
	  			  let _valid = false;
	  			  this.$refs["formtask3"].validate(valid => {
	  			    if (!valid) {
	  			      this.$Message.error("请完善表单信息");
	  			    } else {
	  			      _valid = true;
    ...
    shenpidaiban(){
      let valid = this.validateForm2();
		  ...
		  this.$Modal.confirm({
		  	      ...
		  	      loading: true,
		  	      onOk: () => {
		  	       daiban(this.formModel2.fields).then(res => {
		  	         if (res.data.code === 200) {
		  	           this.$Message.success(res.data.message);
		  	  			 this.loadVoteinitiateList();
		  	  			 this.$Modal.remove();//关闭提示框
...
//编辑按钮权限
    editquanxian(row){
      var show=true;
      if(row.auditStatus=='2'||row.accomplish=="1")
      {
        show=false;
      }
      return show;
      ...
      for (let i = 0; i < this.fil.length; i++) {
        ...
        RegistPicture(formData).then((res) => {
          if (res.data.code == "200") {
            ...
            this.formModel3.fields.fujian = res.data.path;
          } else {
    ...
    handleimportmodel(e) {
      console.log("下载地址");
      console.log(this.url + "UploadFiles/PersonalDiary/" + e);
