...
export default {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.yclactivityUuid);
    }
  },
  methods: {
    ...
    //单条删除
    handleDelete(row) {
      this.doDelete(row.yclactivityUuid);
    },
    doDelete(ids) {
      if (!ids) {
        this.$Message.warning("请选择至少一条数据");
    ...
    handleEdit(row) {
      ...
      this.doLoadPromoInfo(row.yclactivityUuid);
    },
    handleShow(row) {
