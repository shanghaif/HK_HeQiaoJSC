<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="团组织名称" prop="name">
            <Input v-model="formModel.fields.name" :readonly="checkShow()" placeholder="请输入团组织名称" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="团组织书记" prop="secretary">
            <Input
              v-model="formModel.fields.secretary"
              :readonly="checkShow()"
              placeholder="请输入团组织书记"
            />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="其他团干部" prop="cadre">
            <Input
              type="textarea"
              v-model="formModel.fields.cadre"
              :readonly="checkShow()"
              placeholder="请输入其他团干部"
            />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="团员人数" prop="number">
            <Input-number  :min="0" v-model="formModel.fields.number"
              :readonly="checkShow()" style="width:100%;"
              placeholder="请输入团员人数"></Input-number>
          </FormItem>
        </Row>
      </Form>
...
export default {
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "sthOrganicUuid" },
            { title: "序号",type: "index", width: 50},
            { title: "团组织名称", key: "name",ellipsis: true,tooltip: true  },
            { title: "团组织书记", key: "secretary" },
            { title: "其他团干部", key: "cadre",ellipsis: true,tooltip: true },
            { title: "团员人数", key: "number" },
            {
              title: "操作",
              align: "center",
              width: 200,
      ...
      formModel: {
        ...
        rules: {
          name: [{ type: "string", required: true, message: "团组织名称不能为空" }],
          secretary: [{ type: "string", required: true, message: "团组织书记不能为空" }],
          number: [{ type: "number", required: true, message: "团员人数不能为空" }],
        },
      },
    };
  },
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.sthOrganicUuid);
    }, //删除
  },
  methods: {
    //页面加载
    ...
    handleDetail(e) {
      ...
      this.doLoadData(e.sthOrganicUuid);
    },
    //添加按钮
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.sthOrganicUuid);
    },
    //查询当前行信息
    doLoadData(id) {
      GetShow(id).then((res) => {
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.sthOrganicUuid);
    },
    doDelete(ids) {
      if (!ids) {
  ...
  created() {
    ...
    this.actionurl = config.baseUrl.dev + "api/v1/YouthLeague/SOrganic/UpLoad";
  },
};
</script>
<style>
