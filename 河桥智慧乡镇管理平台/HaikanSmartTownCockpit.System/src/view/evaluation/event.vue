      ...
      >
        <Row :gutter="16">
          <FormItem label="活动名称" prop="activityName">
            <Input v-model="formModel.fields.activityName" :readonly="checkShow()" placeholder="请输入活动名称" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="活动时间" prop="activityTime">
            <DatePicker
              v-model="formModel.fields.activityTime"
              @on-change="formModel.fields.activityTime=$event"
              format="yyyy-MM-dd"
              type="date"
              placeholder="请选择活动时间"
              style="width: 100%"
        ...
        <Row :gutter="16">
          <FormItem label="活动方式" prop="activityWay">
            <Input type="textarea" v-model="formModel.fields.activityWay" :readonly="checkShow()" placeholder="请输入活动方式" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="活动地点" prop="address">
            <Input type="textarea" v-model="formModel.fields.address" :readonly="checkShow()" placeholder="请输入活动地点" />
          </FormItem>
        </Row>
      </Form>
...
export default {
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "activityUuid" },
            { title: "活动名称", key: "activityName" },
            { title: "活动时间", key: "activityTime" },
            { title: "活动方式", key: "activityWay", ellipsis: true, tooltip: true },
            { title: "活动地点", key: "address", ellipsis: true, tooltip: true },
            {
              title: "操作",
              align: "center",
              width: 200,
      ...
      formModel: {
        ...
        rules: {
          activityName: [
            { type: "string", required: true, message: "活动名称不能为空" },
          ],
        },
      },
    };
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.activityUuid);
    }, //删除
  },
  methods: {
    //页面加载
    ...
    handleDetail(e) {
      ...
      this.doLoadData(e.activityUuid);
    },
    //添加按钮
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.activityUuid);
    },
    //查询当前行信息
    doLoadData(id) {
      GetShow(id).then((res) => {
    ...
    handleSubmitConsumable() {
      if(this.formModel.fields.activityName!=""){
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.activityName)) {
          this.$Message.warning("活动名称不合法!");
          return;
        }
      }else{
    ...
    doEditDispatch() {
      if (
        this.formModel.fields.activityTime != "" &&
        this.formModel.fields.activityTime != null
      ) {
        this.formModel.fields.activityTime = new Date(
          Date.parse(new Date(this.formModel.fields.activityTime)) + 8 * 3600 * 1000
        );
      }
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.activityUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleimportmodel() {
      window.location.href =
        this.url + "UploadFiles/ImportBuidingModel/支部活动信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
  ...
  created() {
    ...
    this.actionurl = config.baseUrl.dev + "api/v1/Evaluation/Event/UpLoad";
  },
};
</script>
<style>
