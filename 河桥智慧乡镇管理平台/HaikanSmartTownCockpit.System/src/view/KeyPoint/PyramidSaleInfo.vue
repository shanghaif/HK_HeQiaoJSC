  ...
  <div>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="传销地址" prop="otherAddress">
            <Input
        ...
        <Row :gutter="16">
          <FormItem label="违法行为" prop="unlawfulAct">
            <Input
              v-model="formModel.fields.unlawfulAct"
              placeholder="违法行为"
            />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="处罚类别" prop="penaltyType">
            <Input
              v-model="formModel.fields.penaltyType"
              placeholder="处罚类别"
            />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="传销类别" prop="pyramidSaleType">
            <Input
              v-model="formModel.fields.pyramidSaleType"
              placeholder="传销类别"
            />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="传销等级" prop="pyramidSaleClass">
            <Input
              v-model="formModel.fields.pyramidSaleClass"
              placeholder="传销等级"
            />
          </FormItem>
        </Row>
        ...
        <Row :gutter="16">
          <FormItem label="限制时间" prop="bindingHours">
            <Input
              v-model="formModel.fields.bindingHours"
              placeholder="限制时间"
            />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="限制结束时间" prop="bindingHoursEnd">
            <Input
              v-model="formModel.fields.bindingHoursEnd"
              placeholder="限制结束时间"
            />
          </FormItem>
        </Row>
    ...
    <Drawer
      title="传销人员清单导入(导入信息时,性别和生日自动从身份证获取,请保证身份证输入正确)"
...
import {
  ...
  GetImport, //导入
  GetExport, //导出
} from "@/api/KeyPoint/PyramidSaleInfo";
import config from "@/config";
import { getToken } from "@/libs/util";
export default {
  name: "PyramidSaleInfo",
  components: {
    DzTable,
  },
  data() {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.pyramidSaleUuid);
    }, //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.pyramidSaleUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.pyramidSaleUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/KeyPointModel/传销人员清单导入模板.xlsx";
    },
    //导入
    importfxx(e) {
