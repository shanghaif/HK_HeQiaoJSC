  ...
  <div>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="上访原因" prop="reasonForPetition">
            <Input
              v-model="formModel.fields.reasonForPetition"
              placeholder="上访原因"
            />
          </FormItem>
        </Row>
        ...
        <Row :gutter="16">
          <FormItem label="上访状态" prop="petitionStatus">
            <Input
              v-model="formModel.fields.petitionStatus"
              placeholder="上访状态"
            />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="上访类型" prop="petitionType">
            <Input
              v-model="formModel.fields.petitionType"
              placeholder="上访类型"
            />
          </FormItem>
        </Row>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="上访原因" prop="reasonForPetition">
            <Input
              v-model="formModel1.fields.reasonForPetition"
              :readonly="true"
              placeholder="上访原因"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="上访状态" prop="petitionStatus">
            <Input
              v-model="formModel1.fields.petitionStatus"
              :readonly="true"
              placeholder="上访状态"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="上访类型" prop="petitionType">
            <Input
              v-model="formModel1.fields.petitionType"
              :readonly="true"
              placeholder="上访类型"
            />
          </FormItem>
    ...
    <Drawer
      title="重点上访人员清单导入(导入信息时,性别和生日自动从身份证获取,请保证身份证输入正确)"
    ...
    >
      <div>
        ...
        <Button
          ...
          title="重点上访人员清单导入模板下载"
          >重点上访人员清单导入模板下载</Button
        >
        <Button
          icon="md-checkmark-circle"
          type="primary"
...
import {
  ...
  GetImport, //导入
  GetExport, //导出
} from "@/api/KeyPoint/ListOfKeyPetInfo";
...
export default {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.listOfKeyPetitionersUuid);
    }, //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.listOfKeyPetitionersUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.listOfKeyPetitionersUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/KeyPointModel/重点上访人员清单导入模板.xlsx";
    },
    //导入
    importfxx(e) {
