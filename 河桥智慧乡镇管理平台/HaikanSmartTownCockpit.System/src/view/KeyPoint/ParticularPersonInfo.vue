  ...
  <div>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="特定地址" prop="otherAddress">
            <Input
        ...
        <Row :gutter="16">
          <FormItem label="类别" prop="type">
            <Input v-model="formModel.fields.type" placeholder="类别" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="加入时间" prop="joinTime">
            <Input v-model="formModel.fields.joinTime" placeholder="加入时间" />
          </FormItem>
        </Row>
        <Row :gutter="16">
        ...
        <Row :gutter="16">
          <FormItem label="关注原因" prop="attentionCause">
            <Input
              v-model="formModel.fields.attentionCause"
              placeholder="关注原因"
            />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="关系人" prop="privy">
            <Input v-model="formModel.fields.privy" placeholder="关系人" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="关系人电话" prop="privyPhone">
            <Input
              v-model="formModel.fields.privyPhone"
              placeholder="关系人电话"
            />
          </FormItem>
        </Row>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="加入时间" prop="joinTime">
            <Input
              v-model="formModel.fields.joinTime"
              :readonly="true"
              placeholder="加入时间"
            />
          </FormItem>
        ...
        <Row :gutter="16">
          <FormItem label="关系人" prop="privy">
            <Input
              v-model="formModel.fields.privy"
              :readonly="true"
              placeholder="关系人"
            />
          </FormItem>
    ...
    <Drawer
      title="特定人员清单导入(导入信息时,性别和生日自动从身份证获取,请保证身份证输入正确)"
...
import {
  ...
  GetImport, //导入
  GetExport, //导出
} from "@/api/KeyPoint/ParticularPersonInfo";
import config from "@/config";
import { getToken } from "@/libs/util";
export default {
  name: "ParticularPersonInfo",
  components: {
    DzTable,
  },
  data() {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.particularPersonUuid);
    }, //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.particularPersonUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.particularPersonUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/KeyPointModel/特定人员清单导入模板.xlsx";
    },
    //导入
    importfxx(e) {
