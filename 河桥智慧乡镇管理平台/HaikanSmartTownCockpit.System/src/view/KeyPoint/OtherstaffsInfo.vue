  ...
  <div>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="工作情况" prop="workCondition">
            <Input
              v-model="formModel.fields.workCondition"
              placeholder="工作情况"
            />
          </FormItem>
        </Row>
    ...
    <Drawer
      title="其他人员清单导入(导入信息时,性别和生日自动从身份证获取,请保证身份证输入正确)"
...
import {
  ...
  GetImport, //导入
  GetExport, //导出
} from "@/api/KeyPoint/OtherstaffsInfo";
import config from "@/config";
import { getToken } from "@/libs/util";
export default {
  name: "OtherstaffsInfo",
  components: {
    DzTable,
  },
  data() {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.otherstaffsUuid);
    }, //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.otherstaffsUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.otherstaffsUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/KeyPointModel/其他人员清单导入模板.xlsx";
    },
    //导入
    importfxx(e) {
