<template>
  <div>
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="人员类型" prop="personType">
            <Input
              v-model="formModel.fields.personType"
              :readonly="checkShow()"
              placeholder="人员类型"
            />
          </FormItem>
    ...
    >
      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="联系电话" prop="contactNumber">
            <Input
              v-model="formModel1.fields.contactNumber"
              :readonly="true"
              placeholder="联系电话"
            />
          </FormItem>
    ...
    <Drawer
      title="重点青少年信息导入(导入信息时,性别和生日自动从身份证获取,请保证身份证输入正确)"
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "keyYouthListUuid" },
            {
              title: "姓名",
              key: "name",
              minWidth: 120,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.keyYouthListUuid);
    }, //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.keyYouthListUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.keyYouthListUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/KeyPointModel/重点青少年信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
