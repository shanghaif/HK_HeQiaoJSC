      ...
      <Row :gutter="15">
        <Col span="4">
          ...
          <ul class="role-list-box">
            <li :class="{'active': r.selected}" v-for="r in stores.role.data" @click="handleRoleClicked(r)">{{r.roleName}}</li>
          </ul>
        </Col>
        <Col span="20">
          <div style="margin-bottom:10px;">
            ...
            <span style="display:inline-block;position:absolute;right:0;top:1px;">
              <Button icon="md-checkmark-circle" type="primary" @click="handleSaveRolePermissions" v-can="'save'">保 存</Button>
            </span>
          </div>
...
export default {
  ...
  methods: {
    ...
    handleRoleClicked(role) {
      ...
      this.currentRoleCode = role.systemRoleUuid;
      role.selected = true;
