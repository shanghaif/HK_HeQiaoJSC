<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="景点名称" prop="jindianName">
            <Input v-model="formModel.fields.jindianName" placeholder="景点名称" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="容量" prop="rongliang">
              <Input v-model="formModel.fields.rongliang" placeholder="容量" />
            </FormItem>
          </Col>
          <Col span="12">
            <FormItem label="实时人数" prop="ssRenCount">
              <Input v-model="formModel.fields.ssRenCount" placeholder="实时人数" />
            </FormItem>
          </Col>
        </Row>
        <Row :gutter="16">
          <FormItem label="溢出情况" prop="overflowInfo">
            <Input v-model="formModel.fields.overflowInfo" placeholder="溢出情况" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="预警情况" prop="yujinInfo">
            <Input v-model="formModel.fields.yujinInfo" placeholder="预警情况" />
          </FormItem>
        </Row>
      </Form>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="景点名称" prop="jindianName">
            <Input v-model="formModel1.fields.jindianName" placeholder="景点名称" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="容量" prop="rongliang">
              <Input v-model="formModel1.fields.rongliang" placeholder="容量" :readonly="true" />
            </FormItem>
          </Col>
          <Col span="12">
            <FormItem label="实时人数" prop="ssRenCount">
              <Input v-model="formModel1.fields.ssRenCount" placeholder="实时人数" :readonly="true" />
            </FormItem>
          </Col>
        </Row>
        <Row :gutter="16">
          <FormItem label="溢出情况" prop="overflowInfo">
            <Input v-model="formModel1.fields.overflowInfo" placeholder="溢出情况" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="预警情况" prop="yujinInfo">
            <Input v-model="formModel1.fields.yujinInfo" placeholder="预警情况" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
      <div class="demo-drawer-footer">
...
import {
  ...
  GetImport, //导入
  GetExport //导出
} from "@/api/Sightseer/YoukeOverflow";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "youkeOverflowUuid" },
            {
              title: "景点名称",
              key: "jindianName",
              minWidth: 120,
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.youkeOverflowUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.youkeOverflowUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.youkeOverflowUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.jindianName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.jindianName)) {
      ...
      if (this.formModel.fields.ssRenCount != "") {
        let reg = /^[0-9]*[1-9][0-9]*$/;
        if (!reg.test(this.formModel.fields.ssRenCount)) {
          this.$Message.warning("人数格式不正确!");
          return;
        }
      }
      if (this.formModel.fields.rongliang != "") {
        let reg = /^[0-9]*[1-9][0-9]*$/;
        if (!reg.test(this.formModel.fields.rongliang)) {
          this.$Message.warning("容量不能小于0!");
          return;
        }
      }
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/游客溢出情况导入模板.xlsx";
    },
    //导入
    importfxx(e) {
