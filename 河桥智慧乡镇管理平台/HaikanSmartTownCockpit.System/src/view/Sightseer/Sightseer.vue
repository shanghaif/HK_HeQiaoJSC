<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="姓名" prop="sightseerName">
            <Input v-model="formModel.fields.sightseerName" placeholder="姓名" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <Col span="12">
        ...
        <Row :gutter="16">
          <FormItem label="年龄" prop="age">
            <Input v-model="formModel.fields.age" placeholder="年龄" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="手机" prop="phone">
            <Input v-model="formModel.fields.phone" placeholder="手机" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="来源地" prop="laiyuandi">
            <Input v-model="formModel.fields.laiyuandi" placeholder="来源地" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="是否省内人员" prop="shengneiwai">
            <Select
              filterable
              v-model="formModel.fields.shengneiwai"
              style="width:180px"
              placeholder="是否省内人员"
            >
              <Option
                v-for="item in stores.userinfo.sources.shengneiwai"
                :value="item.value"
                :key="item.value"
              >{{ item.label }}</Option>
            </Select>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="姓名" prop="sightseerName">
            <Input v-model="formModel1.fields.sightseerName" placeholder="姓名" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="民族" prop="nation">
            <Input v-model="formModel1.fields.nation" placeholder="民族" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="性别" prop="sex">
        ...
        <Row :gutter="16">
          <FormItem label="身份证号" prop="identityCard">
            <Input v-model="formModel1.fields.identityCard" placeholder="身份证号" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="年龄" prop="age">
            <Input v-model="formModel1.fields.age" placeholder="年龄" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="手机" prop="phone">
            <Input v-model="formModel1.fields.phone" placeholder="手机" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="来源地" prop="laiyuandi">
            <Input v-model="formModel1.fields.laiyuandi" placeholder="来源地" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="是否省内人员" prop="shengneiwai">
            <Input v-model="formModel1.fields.shengneiwai" placeholder="是否省内人员" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
      <div class="demo-drawer-footer">
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "sightseerUuid" },
            {
              title: "姓名",
              key: "sightseerName",
              minWidth: 80,
            ...
            { title: "手机", key: "phone", minWidth: 80, sortable: true },
            {
              title: "身份证号",
              key: "identityCard",
              minWidth: 100,
            ...
            { title: "年龄", key: "age", minWidth: 80, sortable: true },
            { title: "来源地", key: "laiyuandi", minWidth: 80, sortable: true },
            { title: "民族", key: "nation", minWidth: 80, sortable: true },
            {
              title: "是否省内人员",
              key: "shengneiwai",
              minWidth: 80,
      ...
      formModel: {
        ...
        rules: {
          ...
          shengneiwai: [
            { type: "string", required: true, message: "请选择是否省内外人员" }
          ],
        }
      },
      formModel1: {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.sightseerUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.sightseerUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.sightseerUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.sightseerName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.sightseerName)) {
      ...
      if (
        this.formModel.fields.identityCard != null &&
        this.formModel.fields.identityCard != ""
      ) {
      ...
      if (this.formModel.fields.age != "") {
        let reg3 = /^[0-9]*[1-9][0-9]*$/;
        if (!reg3.test(this.formModel.fields.age)) {
          this.$Message.warning("年龄输入不合法!");
          return;
        }
      }
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/游客库导入模板.xlsx";
    },
    //导入
    importfxx(e) {
