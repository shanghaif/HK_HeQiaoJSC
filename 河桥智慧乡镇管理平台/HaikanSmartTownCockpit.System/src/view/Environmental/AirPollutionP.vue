<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="时间" prop="barTime">
            <DatePicker
              v-model="formModel.fields.barTime"
              format="yyyy-MM-dd"
              type="date"
              placeholder="时间"
              style="width: 150px"
        ...
        <Row :gutter="16">
          <FormItem label="当日数值" prop="nowShuzhi">
            <Input v-model="formModel.fields.nowShuzhi" placeholder="当日数值" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="临界点" prop="linjie">
            <Input v-model="formModel.fields.linjie" placeholder="临界点" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="时间" prop="barTime">
            <DatePicker
              v-model="formModel1.fields.barTime"
              format="yyyy-MM-dd"
              type="date"
              placeholder="时间"
              style="width: 150px"
        ...
        <Row :gutter="16">
          <FormItem label="当日数值" prop="nowShuzhi">
            <Input v-model="formModel1.fields.nowShuzhi" placeholder="当日数值" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="临界点" prop="linjie">
            <Input v-model="formModel1.fields.linjie" placeholder="临界点" :readonly="true" />
          </FormItem>
        </Row>
      </Form>
      <div class="demo-drawer-footer">
...
import {
  ...
  GetImport, //导入
  GetExport //导出
} from "@/api/Environmental/AirPollutionP";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "barometricUuid" },
            {
              title: "时间",
              key: "barTime",
              minWidth: 120,
      ...
      formModel: {
        ...
        rules: {
          ...
          nowShuzhi: [
            { type: "string", required: true, message: "数值不能为空" }
          ]
        }
      },
      formModel1: {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.barometricUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.barometricUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.barometricUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/大气防治情况导入模板.xlsx";
    },
    //导入
    importfxx(e) {
