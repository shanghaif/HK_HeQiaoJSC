<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="游客姓名" prop="touristName">
            <Input v-model="formModel.fields.touristName" placeholder="游客姓名" />
          </FormItem>
        </Row>
        <Row :gutter="16">
        ...
        <Row :gutter="16">
          <FormItem label="金额" prop="touristMoney" >
            <!-- <Input v-model="formModel.fields.touristMoney" placeholder="金额" /> -->
          <InputNumber :max="99999999" :min="1"  :step="1.2"  v-model="formModel.fields.touristMoney" style="width:200px"></InputNumber>
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label=" 时间" prop="touristTime">
            <DatePicker
              v-model="formModel.fields.touristTime"
              @on-change="formModel.fields.touristTime=$event"
              format="yyyy-MM-dd"
              type="date"
              placeholder="时间"
              style="width: 150px"
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="游客姓名" prop="touristName">
            <Input v-model="formModel1.fields.touristName" placeholder="游客姓名" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="景点名称" prop="scenicName">
        ...
        <Row :gutter="16">
          <FormItem label="金额" prop="touristMoney">
            <Input v-model="formModel1.fields.touristMoney" placeholder="金额" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="时间" prop="touristTime">
            <DatePicker
              v-model="formModel1.fields.touristTime"
              format="yyyy-MM-dd"
              type="date"
              placeholder="时间"
              style="width: 150px"
...
import {
  ...
  GetImport, //导入
  GetExport //导出
} from "@/api/JingdianInfo/lvyouinfo";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "touristUuid" },
            {
              title: "游客姓名",
              key: "touristName",
              minWidth: 120,
      ...
      formModel: {
        ...
        rules: {
          touristName: [
            { type: "string", required: true, message: "游客姓名不能为空" }
          ],
          scenicName: [
          ...
          touristMoney: [
            { type: "number", required: true, message: "金额不能为空" }
          ]
        }
      },
      formModel1: {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.touristUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.touristUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    doLoadData(id) {
      GetfoGet(id).then(res => {
        ...
        console.log(this.formModel.fields.touristMoney);
      });
    },
    validateUserForm() {
      let _valid = false;
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.touristName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.touristName)) {
          this.$Message.warning("游客姓名不合法!");
          return;
        }
      } else {
        this.$Message.warning("游客姓名不能为空!");
        return;
      }
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/游客信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
