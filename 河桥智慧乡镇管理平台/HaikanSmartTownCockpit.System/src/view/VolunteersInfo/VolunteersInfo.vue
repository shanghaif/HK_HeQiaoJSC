<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="志愿者姓名" prop="volunteersName">
            <Input v-model="formModel.fields.volunteersName" placeholder="志愿者姓名" />
          </FormItem>
        </Row>
                <Row :gutter="16">
        ...
        <Row :gutter="16">
          <FormItem label="个人简介" prop="jieshao">
            <Input v-model="formModel.fields.jieshao" type="textarea" placeholder="个人简介" />
          </FormItem>
        </Row>
      </Form>
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="志愿者姓名" prop="volunteersName">
            <Input v-model="formModel1.fields.volunteersName" placeholder="志愿者姓名" :readonly="true" />
          </FormItem>
        </Row>
        ...
        <Row :gutter="16">
          <FormItem label="个人简介" prop="jieshao">
            <Input v-model="formModel1.fields.jieshao" placeholder="个人简介" :readonly="true" />
          </FormItem>
        </Row>
...
import {
  ...
  GetImport, //导入
  GetExport //导出
} from "@/api/VolunteersInfo/VolunteersInfo";
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "volunteersUuid" },
            {
              title: "志愿者姓名",
              key: "volunteersName",
              minWidth: 120,
      ...
      formModel: {
        ...
        rules: {
          volunteersName: [
            { type: "string", required: true, message: "请输入志愿者姓名" }
          ],
        }
      },
      formModel1: {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.volunteersUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.volunteersUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.volunteersUuid);
    },
    //右边详情
    handleDetail(row) {
    ...
    //添加（保存）
    docreateDispatch() {
      if (this.formModel.fields.volunteersName != "") {
        let reg = /^[^\s]+$/;
        if (!reg.test(this.formModel.fields.volunteersName)) {
          this.$Message.warning("请勿输入非法字符!");
          return;
        }
      } else {
        this.$Message.warning("志愿者姓名不能为空!");
        return;
      }
    ...
    handleimportmodel() {
      ...
      window.location.href =
        this.url + "UploadFiles/ImportUserInfoModel/侨界志愿者信息导入模板.xlsx";
    },
    //导入
    importfxx(e) {
