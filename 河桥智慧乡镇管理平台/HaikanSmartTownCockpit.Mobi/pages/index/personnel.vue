			...
			<view class="cu-list menu">
				...
				<checkbox-group class="cu-list menu" @change="changeCheckbox">
					<view class="cu-item" v-for="(item,index) in checkbox" :key="index">
						<view class="action">
							<checkbox class='round' :value="item.id" :checked="checkedArr.includes(item.id)" style="font-size: 13px; width: 20px; height: 20px;align-items:center"></checkbox>
						...
						<view class="action">
							<text class="text-grey text-sm" style="color: #000000;">
								<button @click="showDrawer(item.id)" :disabled="checkedArr.includes(item.id+'&'+item.name)" :style="checkedArr.includes(item.id+'&'+item.name)==true?'color:#C0C0C0':'color:rgb(0, 122, 255)'"
								 style="    color: rgb(0, 122, 255);    display: inline-block; background: #FFFFFF; height: 15px; line-height: 15px; border: 0px;   font-size: 13px;    margin: auto;    position: initial;    overflow: inherit;    padding: inherit;    line-height: initial;">下级</button></text>
						</view>
					</view>
				</checkbox-group>
			</view>
		...
		<uni-drawer ref="showRight" mode="right" :mask-click="false" @change="change($event,'showRight')">
			<view class="scroll-view" :style="">
				<scroll-view class="scroll-view-box" scroll-y="true">
					...
					<checkbox-group class="cu-list menu" @change="changeCheckboxs" style="border-top: 0.5px solid #DDDDDD;margin-top:0px;margin-bottom: 145px;">
						<view class="cu-item" v-for="(item,index) in checkboxs" :key="index">
							<view class="action">
								<checkbox class='round' :value="item.id+'&'+item.name+'&'+item.uid" :checked="checkedArrs.includes(item.id+'&'+item.name+'&'+item.uid)"
					...
					<div id="button_divs">
						<view class="close" style="padding: 0px;">
							...
							<view class="word-btn" hover-class="word-btn--hover" :hover-start-time="20" :hover-stay-time="70" @click="closeDrawer1('showRight')"><text
	...
	export default {
		...
		methods: {
			...
			getuserinfosx() {
				...
				getuserlists(strlist).then(res => {
					console.log("日志信息：");
					console.log(res);
					for (let item1 of res.data.data) {
						let itemVal = item1.id + '&' + item1.name + '&' + item1.uid;
			...
			closeDrawer(e) {
				...
				console.log(that.checkedArrs);
				//当前部门没有的人				
				for (let item1 of that.checkboxs) {
					let values = item1.id + "&" + item1.name + '&' + item1.uid;
					that.checkedArrs14.push(values);
				}
				if (that.checkedArrs11 != "") {
					for (let item of that.checkedArrs11) {
				...
				if (that.checkedArrs != "") {
					for (let item of that.checkedArrs) {
						that.checkedArrs13.push(item);
					}
				}
				if (that.checkedArrs12 != "") {
					for (let item of that.checkedArrs12) {
						that.checkedArrs13.push(item);
					}
				}
				console.log("最后返回的");
			...
			allChooses(e) {
				...
				if (chooseItem[0] == 'alls') {
					...
					for (let item1 of that.checkboxs) {
						let itemVal = String(item1.id + '&' + item1.name + '&' + item1.uid);
						if (!that.checkedArrs.includes(itemVal)) {
							that.checkedArrs.push(itemVal);
						}
					}
